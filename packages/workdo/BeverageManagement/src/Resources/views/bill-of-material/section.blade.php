...
<script>
    var bill_material_id = '{{ $bill_material_id }}';
                ...
                $.ajax({
                    ...
                    success: function(data) {
                        ...
                        if (invoiceItems != null) {
                            ...
                            $(el.parent().parent().find('.bill_material_item_id')).val(invoiceItems
                                .id);
                        } else {
                            $(el.parent().parent().find('.quantity')).val(1);
                            $(el.parent().parent().find('.price')).val(item.product.sale_price);
    ...
    $(document).on('click', '[data-repeater-delete]', function() {
        ...
        var bill_material_item_id = $(this).closest('tr').find('.bill_material_item_id').val();
        if(bill_material_item_id){
            $.ajax({
                url: "{{ route('raw.material.delete') }}",
                type: 'POST',
                data: {
                    _token: "{{ csrf_token() }}",
                    'bill_material_item_id': bill_material_item_id
                },
                cache: false,
                success: function(data) {
                    if (data.success) {
...
<div class="card repeater" @if ($action=='edit' ) data-value='{!! json_encode($bill_material_item_summary) !!}' @endif>
                ...
                <tbody class="ui-sortable" data-repeater-item>
                    <tr>
                        <td width="25%" class="form-group pt-0">
                            {{ Form::hidden('bill_material_item_id', '', ['class' => 'form-control bill_material_item_id']) }}
                            ...
                            <select class="form-control raw_material_id" name="raw_material_id" id="raw_material_id" placeholder="Select Raw Material" data-url="{{ route('raw.material') }}" required>
                                <option value="">{{__('Select Raw Material Parts')}}</option>
                                @foreach($raw_materials as $raw_material)
                                <option value="{{ $raw_material->id }}">{{ $raw_material->productService->name }} - ({{ $raw_material->productService->type }})</option>
                                @endforeach
                            </select>
                            <div class="text-muted text-xs mt-1">{{ __('Please add Items Here') }} <a href="{{ route('raw-material.index') }}">{{ __('Add Parts') }}</a></div>
                        </td>
                        <td>
