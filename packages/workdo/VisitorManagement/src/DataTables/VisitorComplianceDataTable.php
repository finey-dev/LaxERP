...
class VisitorComplianceDataTable extends DataTable
{
    /**
    ...
    public function dataTable(QueryBuilder $query): EloquentDataTable
    {
        ...
        $dataTable = (new EloquentDataTable($query))
            ->addIndexColumn()
            ->editColumn('visitor_id', function (VisitorCompliance $visitor_compliance) {
                return !empty($visitor_compliance->visitor_id) ? $visitor_compliance->visitor->first_name : '';
            })
            ->editColumn('compliance_type', function (VisitorCompliance $visitor_compliance) {
                return !empty($visitor_compliance->compliance_type) ? $visitor_compliance->compliance->name : '-';
            })

            ->editColumn('status', function (VisitorCompliance $visitor_compliance) {
                if ($visitor_compliance->status == 0) {
                    $html = "<span class='badge bg-info p-2 px-3'>" . __('Pending') . "</span>";
                } else if ($visitor_compliance->status == 1) {
            ...
            ->editColumn('date', function (VisitorCompliance $visitor_compliance) {
                return company_datetime_formate($visitor_compliance->date);
            })
        ...
        if (\Laratrust::hasPermission('visitor compliance delete') || \Laratrust::hasPermission('visitor compliance edit')) {
            $dataTable->addColumn('action', function (VisitorCompliance $visitor_compliance) {
                return view('visitor-management::visitor-compliance.action', compact('visitor_compliance'));
            });
            $rowColumn[] = 'action';
        }
     ...
     */
    public function query(VisitorCompliance $model): QueryBuilder
    {
        $query = $model->with(['visitor','compliance'])
            ->where('visitor_compliances.created_by', creatorId())
            ->where('visitor_compliances.workspace', getActiveWorkSpace());
        // dd($query->get());
        return $query;
    }
    ...
    public function html(): HtmlBuilder
    {
        $dataTable = $this->builder()
            ->setTableId('visitor_compliances-table')
    ...
    public function getColumns(): array
    {
        $column = [
            ...
            Column::make('compliance_type')->title(__('Complince Type'))->name('compliance.name'),
            Column::make('date')->title(__('Compliance Date')),
    ...
    {
        return 'visitor_compliance__' . date('YmdHis');
    }
}
