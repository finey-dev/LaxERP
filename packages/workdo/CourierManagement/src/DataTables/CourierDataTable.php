...
class CourierDataTable extends DataTable
{
    ...
    public function dataTable(QueryBuilder $query): EloquentDataTable
    {
        ...
        $dataTable = (new EloquentDataTable($query))
            ...
            ->editColumn('package_title', function (CourierReceiverDetails $courier) {
                return $courier->packageInformarmation ? $courier->packageInformarmation->package_title ?? '-' : '--';
            })
            ->editColumn('created_by', function (CourierReceiverDetails $courier) {
                return $courier->createdBy->name;
            })
            ->editColumn('tracking_status', function (CourierReceiverDetails $courier) {
                $statusColor = $courier->packageInformarmation->getTrackingStatus->status_color ?? 'ffa833';
                $statusName = $courier->packageInformarmation->getTrackingStatus->status_name ??  'pending';
            ...
            ->filterColumn('tracking_status', function ($query, $keyword) {
                if (stripos('pending', $keyword) !== false) {
                    return $query->whereHas('courier_package_infos', function ($query) {
                        $query->where('tracking_status', 'pending');
                    });
                }
            })
            ->editColumn('payment_type', function (CourierReceiverDetails $courier) {
                return $courier->payment_type ?? '---';
            })
            ->editColumn('payment_status', function (CourierReceiverDetails $courier) {
                $statusClass = '';
                $paymentStatus = $courier->payment_status;
                ...
                $html = '<span class="badge fix_badge ' . $statusClass . ' p-2 px-3">' . $paymentStatus . '</span>';
                return $html;
            })
        ...
        if ((\Laratrust::hasPermission('courier manage')) || (\Laratrust::hasPermission('courier payment'))  || (\Laratrust::hasPermission('courier edit')) || (\Laratrust::hasPermission('courier delete'))) {
            $dataTable->addColumn('action', function (CourierReceiverDetails $courier) {
                $workspace = WorkSpace::where('id', getActiveWorkSpace())->first();
                return view('courier-management::courier.action', compact('courier', 'workspace'));
            });
            $rowColumn[] = 'action';
        }
    ...
    public function getColumns(): array
    {
        $column = [
            ...
            Column::make('package_title')->title(__('Package Title'))->name('packageInformarmation.package_title'),
            ...
            Column::make('tracking_status')->title(__('Tracking Status'))->name('packageInformarmation.getTrackingStatus.status_name')->orderable(false),
            Column::make('category')->title(__('Category'))->name('packageInformarmation.courier_category.category'),
