...
@section('page-action')
<div class="d-flex">
    <a href="{{ route('rfx.index') }}" class="btn btn-sm btn-primary me-2" data-bs-toggle="tooltip"
        title="{{ __('List View') }}">
...
<div class="row row-gap-2 mb-4">
                ...
                <div class="d-flex align-items-center">
                    <div class="user-img-wrp d-flex align-items-center">
                        <div class="user-image rounded border-2 border border-primary">
                            <img alt="user-image" class="h-100 w-100" avatar="{{ $rfx->title }}">
                        </div>
                        <div class="user-content">
                            <h6 class="mb-0">
                                <a class=" text-primary"
                                    href="{{ route('rfx.show', $rfx->id) }}">{{ ucfirst($rfx->title) }}</a>
                            </h6>
                            <div class="mb-3"><a class="text-sm small text-muted">{{ $rfx->location }}</a></div>
                        </div>
                    </div>
                </div>
                <div class="card-header-right">
                    <div class="btn-group card-option">
                        ...
                        <div class="dropdown-menu dropdown-menu-end">
                            @if (Laratrust::hasPermission('rfx show') ||
                            Laratrust::hasPermission('rfx edit') ||
                            Laratrust::hasPermission('rfx delete'))
                            @permission('rfx show')
                            <a href="{{ route('rfx.show', $rfx->id) }}" class="dropdown-item"
                                data-bs-whatever="{{ __('Rfx Details') }}" data-bs-toggle="tooltip"
                                data-title="{{ __('Rfx Details') }}"><i class="ti ti-eye me-1"></i>
                                {{ __('View') }}</a>
                            @endpermission

                            @permission('rfx edit')
                            <a href="{{ route('rfx.edit', $rfx->id) }}" data-size="md" class="dropdown-item"
                                data-bs-whatever="{{ __('Edit') }}" data-bs-toggle="tooltip"
                                data-title="{{ __('Edit') }}"><i class="ti ti-pencil me-1"></i>
                                {{ __('Edit') }}</a>
                            @endpermission

                            @permission('rfx delete')
                            {{ Form::open(['route' => ['rfx.destroy', $rfx->id], 'class' => 'm-0']) }}
                            @method('DELETE')
                            <a href="#!" class="dropdown-item bs-pass-para show_confirm" aria-label="Delete"
                                data-confirm="{{ __('Are You Sure?') }}"
                                data-text="{{ __('This action can not be undone. Do you want to continue?') }}"
                                data-confirm-yes="delete-form-{{ $rfx->id }}">
                            ...
                            <a href="#"
                                ...
                                title="{{ __('Copy') }}" data-original-title="{{ __('Click to copy link') }}"><i
                ...
                <div class="bottom-icons d-flex flex-wrap align-items-center justify-content-between">
                    <div class="edit-btn-wrp d-flex flex-wrap align-items-center">
                        @if (Laratrust::hasPermission('rfx show') ||
                            ...
                            Laratrust::hasPermission('rfx delete'))
                            @permission('rfx edit')
                            <a href="{{ route('rfx.edit', $rfx->id) }}" data-size="md" class="btn btn-sm border"
                                data-bs-whatever="{{ __('Edit') }}" data-bs-toggle="tooltip"
                                title="{{ __('Edit') }}">
                            ...
                            {{ Form::open(['route' => ['rfx.destroy', $rfx->id], 'class' => 'm-0']) }}
                            @method('DELETE')
                            <a href="#!" class="btn btn-sm border  show_confirm" data-bs-toggle="tooltip" title=""
                            ...
                            @if ($rfx->status != 'in_active')
                                <a href="#"
                                    id="{{ route('rfx.requirement', [$rfx->code, !empty($rfx) ? $rfx->createdBy->lang : 'en']) }}"
                                    class="btn btn-sm border" onclick="copyToClipboard(this)" data-bs-toggle="tooltip"
                                    title="{{ __('Copy') }}" data-original-title="{{ __('Click to copy link') }}">
                    ...
                    <span
                        class="badge bg-success p-2 px-3">{{ Workdo\Procurement\Entities\Rfx::$status[$rfx->status] }}</span>
                    @else
                    <span
                        class="badge bg-danger p-2 px-3">{{ Workdo\Procurement\Entities\Rfx::$status[$rfx->status] }}</span>
                    @endif
                </div>
            </div>
        </div>
    ...
    @permission('rfx create')
    <div class="col-xxl-3 col-xl-4 col-md-6">
        <a href="{{ route('rfx.create') }}" class="btn-addnew-project border-primary"
            data-title="{{ __('Create RFx') }}">
            <div class="bg-primary proj-add-icon">
                <i class="ti ti-plus my-2"></i>
            </div>
            <h6 class="my-2 text-center">{{ __('RFx') }}</h6>
            <p class="text-muted text-center">{{ __('Click here to add New rfx') }}</p>
        </a>
    </div>
    @endpermission
</div>
{!! $rfxs->links('vendor.pagination.global-pagination') !!}
@endsection
