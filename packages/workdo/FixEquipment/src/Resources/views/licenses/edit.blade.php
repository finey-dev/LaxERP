{{ Form::open(['route' => ['fix.equipment.licence.update', $license->id],'class'=>'needs-validation','novalidate']) }}
<div class="modal-body">
    <div class="row">
        <div class="col-md-12">
            <div class="form-group">
                {{ Form::label('Title', __('Title'), ['class' => 'form-label']) }}<x-required></x-required>
                {{ Form::text('title', $license->title, ['class' => 'form-control', 'required' => 'required', 'placeholder' => __('Enter Title')]) }}
            </div>
        </div>
        <div class="col-md-12">
            <div class="form-group">
                ...
                <select name="category" id="category" class="form-control" required>
                    ...
                    @foreach ($categories as $category)
                        <option value="{{ $category->id }}" {{ $license->category == $category->id ? 'selected' : '' }}>
            ...
            <div class="form-group">
                ...
                <select name="asset" id="asset" class="form-control" required>
                    ...
                    @foreach ($assets as $asset)
                        <option value="{{ $asset->id }}" {{ $license->asset == $asset->id ? 'selected' : '' }}>
                            {{ $asset->title }}</option>
                    @endforeach
                </select>
            </div>
        ...
        <div class="col-md-12">
            <div class="form-group">
                ...
                {{ Form::text('license_number', $license->license_number, ['class' => 'form-control', 'required' => 'required', 'placeholder' => __('Enter License Number')]) }}
            </div>
        </div>
        <div class="col-md-6">
            <div class="form-group">
                {{ Form::label('Purchase Date', __('Purchase Date'), ['class' => 'form-label']) }}<x-required></x-required>
                {{ Form::date('purchase_date', $license->purchase_date, ['class' => 'form-control', 'required' => 'required']) }}
            </div>
        </div>
        <div class="col-md-6">
            <div class="form-group">
                {{ Form::label('Expire Date', __('Expire Date'), ['class' => 'form-label']) }}<x-required></x-required>
                {{ Form::date('expire_date', $license->expire_date, ['class' => 'form-control', 'required' => 'required']) }}
            </div>
        </div>
        <div class="col-md-12">
            <div class="form-group">
                {{ Form::label('Purchase Price', __('Purchase Price'), ['class' => 'form-label']) }}<x-required></x-required>
                {{ Form::number('purchase_price', $license->purchase_price, ['class' => 'form-control', 'required' => 'required', 'placeholder' => __('Enter Price')]) }}
            </div>
        </div>
