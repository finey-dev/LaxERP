...
class LicensesDataTable extends DataTable
{
    ...
    {
        $rowColumn = ['category','asset','purchase_price','purchase_date','expire_date'];
        $dataTable = (new EloquentDataTable($query))
        ->addIndexColumn()
        ->editColumn('category', function (AssetLicense $license) {
            return optional($license->equipmentCategory)->title ?? '';
        })
        ->editColumn('asset', function (AssetLicense $license) {
            return optional($license->fixAsset)->title ?? '';
        })
        ->editColumn('purchase_price', function (AssetLicense $license) {
            return currency_format_with_sym($license->purchase_price);
        })
        ->editColumn('purchase_date', function (AssetLicense $license) {
            return  company_date_formate($license->purchase_date);
        })
        ->editColumn('expire_date', function (AssetLicense $license) {
            return  company_date_formate($license->expire_date);
        })
        ...
        ->filterColumn('expire_date', function ($query, $keyword) {
            try {
                if (\Carbon\Carbon::hasFormat($keyword, 'd-m-Y')) {
                    $date = \Carbon\Carbon::createFromFormat('d-m-Y', $keyword)->format('Y-m-d');
                    return $query->where('expire_date', 'LIKE', "%$date%");
                } elseif (\Carbon\Carbon::hasFormat($keyword, 'm-Y')) {
                    $date = \Carbon\Carbon::createFromFormat('m-Y', $keyword)->format('Y-m');
                    return $query->where('expire_date', 'LIKE', "%$date%");
                } elseif (\Carbon\Carbon::hasFormat($keyword, 'd-m')) {
                    ...
                    if (\Carbon\Carbon::hasFormat($keyword, 'd') && strlen($keyword) <= 2) {
                        $day = \Carbon\Carbon::createFromFormat('d', $keyword)->format('d');
                        $query->whereRaw('DAY(expire_date) = ?', [$day]);
                        $hasDay = true;
                    }
                    if (\Carbon\Carbon::hasFormat($keyword, 'm') && strlen($keyword) <= 2) {
                        $month = \Carbon\Carbon::createFromFormat('m', $keyword)->format('m');
                        $query->orWhereRaw('MONTH(expire_date) = ?', [$month]);
                        $hasMonth = true;
                    }
                    if (preg_match('/^\d{4}$/', $keyword)) {
                        $year = \Carbon\Carbon::createFromFormat('Y', $keyword)->format('Y');
                        $query->orWhereRaw('YEAR(expire_date) = ?', [$year]);
                        $hasYear = true;
                    }
        ...
        if (\Laratrust::hasPermission('asset licenses manage') ||
            \Laratrust::hasPermission('asset licenses edit') ||
            \Laratrust::hasPermission('asset licenses delete')) {

            $dataTable->addColumn('action', function (AssetLicense $license) {
                return view('fix-equipment::licenses.action', compact('license'));
            });
            $rowColumn[] = 'action';
        }
     ...
     */
    public function query(AssetLicense $model)
    {
        $licenses = $model->with('equipmentCategory', 'fixAsset')->where('workspace', getActiveWorkSpace())->where('created_by', creatorId());
        return $licenses;
    }
    ...
    public function html(): HtmlBuilder
    {
        $dataTable = $this->builder()
            ->setTableId('asset_licenses-table')
    ...
    public function getColumns(): array
    {
        ...
        $column =  [
            ...
            Column::make('expire_date')->title(__('Expiry Date')),
