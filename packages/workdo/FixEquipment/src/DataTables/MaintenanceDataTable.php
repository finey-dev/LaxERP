...
class MaintenanceDataTable extends DataTable
{
    ...
    {
        $rowColumn = ['asset','maintenance_date','price'];
        $dataTable = (new EloquentDataTable($query))
        ->addIndexColumn()

        ->editColumn('asset', function (Maintenance $maintenance) {
            return optional($maintenance->fixAsset)->title ?? '';
        })
        ->editColumn('maintenance_date', function (Maintenance $maintenance) {
            return company_date_formate($maintenance->maintenance_date);
        })
        ...
        ->editColumn('price', function (Maintenance $maintenance) {
            return currency_format_with_sym($maintenance->price);
        })
        ;

        if (\Laratrust::hasPermission('equipment maintenance manage') ||
            \Laratrust::hasPermission('equipment maintenance edit') ||
            \Laratrust::hasPermission('equipment maintenance delete')) {

            $dataTable->addColumn('action', function (Maintenance $maintenance) {
                return view('fix-equipment::maintenance.action', compact('maintenance'));
            });
            $rowColumn[] = 'action';
        }
    ...
    {
        $maintenaces = $model->with('fixAsset')->where('workspace', getActiveWorkSpace())->where('created_by', creatorId());
