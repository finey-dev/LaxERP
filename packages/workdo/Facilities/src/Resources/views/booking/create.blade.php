{{ Form::open(array('route' => array('facility-booking.store'), 'id' => 'yourFormId','class'=>'needs-validation','novalidate')) }}
    <div class="modal-body">
        <div class="row">
            <div class="form-group col-md-6 col-12 mb-3">
                {{ Form::label('service', __('Service'), ['class' => 'form-label']) }}<x-required></x-required>
                {{ Form::select('service', $service, '', ['class' => 'form-control', 'id' => 'service', 'required' => 'required']) }}
            </div>
            <div class="form-group col-md-6 col-12 mb-3">
                {{ Form::label('date', __('Date'), ['class' => 'form-label']) }}<x-required></x-required>
                {{ Form::date('date', '', ['class' => 'form-control ', 'placeholder' => 'Enter Date', 'id' => 'date','min' => date('Y-m-d'), 'required' => 'required']) }}
            </div>
            <div class="form-group col-md-6 col-12 mb-3">
