...
use Workdo\Recruitment\Events\CreateJobStage;
use Workdo\Recruitment\Events\DestroyJobStage;
use Workdo\Recruitment\Events\UpdateJobStage;
...
class JobStageController extends Controller
{
    ...
    public function store(Request $request)
    {
        if (Auth::user()->isAbleTo('jobstage create')) {
            ...
            return redirect()->back()->with('success', __('The job stage has been created successfully.'));
        } else {
            return redirect()->back()->with('error', __('Permission denied.'));
        }
    }
    ...
    public function update(Request $request, JobStage $jobStage)
    {
        if (Auth::user()->isAbleTo('jobstage edit')) {
            ...
            return redirect()->back()->with('success', __('The job stage details are updated successfully.'));
        } else {
            return redirect()->back()->with('error', __('Permission denied.'));
        }
    }
    ...
    public function destroy(JobStage $jobStage)
    {
        if (Auth::user()->isAbleTo('jobstage delete')) {
            if ($jobStage->created_by == creatorId() && $jobStage->workspace == getActiveWorkSpace()) {
                ...
                return redirect()->back()->with('success', __('The job stage has been deleted.'));
            } else {
                return redirect()->back()->with('error', __('Permission denied.'));
            }
        } else {
