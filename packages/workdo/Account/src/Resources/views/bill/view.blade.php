...
@section('content')
                ...
                <div class="row timeline-wrapper">
                        ...
                        <div class="d-flex align-items-start gap-2">
                            ...
                            <div class="invoice-content">
                                ...
                                @permission('bill edit')
                                    <a href="{{ route('bill.edit',\Crypt::encrypt($bill->id)) }}" class="btn btn-sm btn-light"                  data-bs-toggle="tooltip" data-original-title="{{__('Edit')}}"><i class="mr-2 ti ti-pencil"></i>{{__('Edit')}}
                                    </a>
                                @endpermission
                            </div>
                        </div>
                            ...
                            <div class="invoice-content">
                                ...
                                @if($bill->status==0)
                                    @permission('bill send')
                                        <a href="{{ route('bill.sent',$bill->id) }}" class="btn btn-sm btn-warning" data-bs-toggle="tooltip" data-original-title="{{__('Mark Sent')}}"><i class="me-1 ti ti-send"></i>{{__('Send')}}</a>
                                    @endpermission
                                @endif
                            </div>
                        </div>
                    ...
                    <div class="col-xl-4 col-lg-4 col-sm-6">
                        ...
                        <div class="d-flex align-items-start gap-2">
                            ...
                            <div class="invoice-content">
                                <h6 class="text-info mb-2">{{__('Pay Bill')}}</h6>
                                ...
                                @if($bill->status!=0)
                                    @permission('bill payment create')
                                        <a href="#" data-url="{{ route('bill.payment',$bill->id) }}" data-ajax-popup="true" data-title="{{__('Add Payment')}}" class="btn btn-sm btn-light" data-original-title="{{__('Add Payment')}}"><i class="mr-2 ti ti-report-money"></i>{{__('Add Payment')}}</a> <br>
                                    @endpermission
                                @endif
                            </div>
                        </div>
    ...
    <div class="mb-3 row justify-content-between align-items-center">
        ...
        @else
            <div class="col-md-6 d-flex align-items-center justify-content-between justify-content-md-end">
                <div class="mx-2 all-button-box">
                    <a href="{{route('bill.sent',$bill->id)}}" class="btn btn-sm btn-primary">
                        {{__('Send Mail')}}
                    </a>
                </div>
                <div class="mx-2 all-button-box">
                                            ...
                                            <div class="d-flex invoice-wrp flex-wrap align-items-center gap-md-2 gap-1 justify-content-end">
                                                <div class="d-flex invoice-date flex-wrap align-items-center justify-content-end gap-md-3 gap-1">
                                                    <p class="mb-0"><strong>{{ __('Issue Date') }} :</strong>
                                                        {{ company_date_formate($bill->issue_date) }}</p>
                                    ...
                                    <div class="p-sm-4 p-3 invoice-billed">
                                        <div class="row row-gap">
                                            ...
                                            <div class="col-lg-2 col-sm-6">
                                                <strong class="h5 d-block mb-2">{{__('Status')}} :</strong><br>
                                                @if ($bill->status == 0)
                                                ...
                                                @elseif($bill->status == 2)
                                                    <span class="badge fix_badge f-12 p-2 d-inline-block bg-secondary">{{ __(Workdo\Account\Entities\Bill::$statues[$bill->status]) }}</span>
                                                @elseif($bill->status == 3)
                                        ...
                                        <div class="mt-2 table-responsive">
                                            <table class="table mb-0 table-striped">
                                                <tr>
                                                    ...
                                                    @if ($bill->bill_module == 'account' || $bill->bill_module == '')
                                                    <th class="text-white bg-primary text-uppercase">{{__('Chart Of Account')}}</th>
                                                    <th class="text-white bg-primary text-uppercase">{{__('Account Amount')}}</th>
                                                    @endif
                                                ...
                                                <tfoot>
                                                    <tr>
                                                        ...
                                                        @if ($bill->bill_module == 'account' || $bill->bill_module == '')
                                                            ...
                                                            <td class="bg-color"><b>{{currency_format_with_sym($bill->getAccountTotal())}}</b></td>
                                                        @endif
                                                    </tr>
                                                    @php
                                                        $colspan = 6;
                                            ...
                                            <td>
                                                @if(!empty($payment->add_receipt))
                                                    <div class="action-btn  me-2">
                                                        <a href="{{ get_file($payment->add_receipt)}}" download="" class=" bg-primary mx-3 btn btn-sm align-items-center" data-bs-toggle="tooltip" title="{{__('Download')}}" target="_blank" >
                                                            <i class="text-white ti ti-download" ></i>
                                                        </a>
                                                    </div>
                                                    <div class="action-btn">
                                                        <a href="{{ get_file($payment->add_receipt)}}" class="bg-secondary mx-3 btn btn-sm align-items-center" data-bs-toggle="tooltip" title="{{__('Show')}}" target="_blank" >
                                                ...
                                                @permission('bill payment delete')
                                                    <div class="action-btn">
                                                        {{Form::open(array('route'=>array('bill.payment.destroy',$bill->id,$payment->id),'class' => 'm-0'))}}
                                                            <a href="#" class="bg-danger mx-3 btn btn-sm align-items-center bs-pass-para show_confirm"
                                        ...
                                        <tr>
                                            ...
                                            <td>
                                                @permission('debitnote edit')
                                                    <div class="action-btn  me-2">
                                                        <a data-url="{{ route('bill.edit.debit.note',[$debitNote->bill,$debitNote->id]) }}" data-ajax-popup="true" data-title="{{__('Add Debit Note')}}" href="#" class="bg-info mx-3 btn btn-sm align-items-center" data-bs-toggle="tooltip" data-original-title="{{__('Edit')}}">
                                    ...
                                    <div class="table-responsive">
                                        ...
                                        <thead>
                                            ...
                                            @forelse($bill_attachment as $key =>$attachment)
                                                    ...
                                                    <td>
                                                            <div class="action-btn  me-2">
                                                                <a href="{{ url($attachment->file_path) }}" class="mx-3 btn btn-sm align-items-center bg-primary" title="{{__('Download')}}" target="_blank" download>
                                                                    <i class="text-white ti ti-download"></i>
                                                                </a>
                                                            </div>
                                                            <div class="action-btn">
