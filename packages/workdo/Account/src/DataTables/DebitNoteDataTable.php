...
class DebitNoteDataTable extends DataTable
{
    ...
    {
        $rawColumn = [ 'bill','date', 'vendor', 'amount','status'];
        $dataTable = (new EloquentDataTable($query))
            ...
            ->editColumn('bill', function (CustomerDebitNotes $customdebitNote) {
                if (\Laratrust::hasPermission('bill show')) {
                    ...
                    return '<a href="' . $url . '" class="btn btn-outline-primary">' . Bill::billNumberFormat($customdebitNote->bill) . '</a>';
                } else {
                    return ' <a href="#" class="btn btn-outline-primary">' . Bill::billNumberFormat($customdebitNote->bill) . '</a>';
                }
            })
    ...
    public function query(CustomerDebitNotes $model)
    {
        ...
        $customdebitNotes = $model->with('custom_vendor')->select('customer_debit_notes.*')
            ->leftJoin('bills', 'customer_debit_notes.bill', '=', 'bills.bill_id')
