...
class CreditNoteDataTable extends DataTable
{
    ...
    {
        $rawColumn = ['invoice', 'date', 'customer', 'amount','status'];
        $dataTable = (new EloquentDataTable($query))
            ...
            ->editColumn('invoice', function (CustomerCreditNotes $customcreditNote) {
                if (\Laratrust::hasPermission('invoice show')) {
                    ...
                    return '<a href="' . $url . '" class="btn btn-outline-primary">' . Invoice::invoiceNumberFormat($customcreditNote->invoice) . '</a>';
                } else {
                    return ' <a href="#" class="btn btn-outline-primary">' . Invoice::invoiceNumberFormat($customcreditNote->invoice) . '</a>';
                }
            })
    ...
    public function query(CustomerCreditNotes $model)
    {
       ...
       $customcreditNotes = $model->with('custom_customer')->select('customer_credit_notes.*')
            ->leftJoin('invoices', 'customer_credit_notes.invoice', '=', 'invoices.invoice_id')
    ...
    public function getColumns(): array
    {
        ...
        $column = [
            ...
            Column::make('invoice')->title(__('Invoice')),
