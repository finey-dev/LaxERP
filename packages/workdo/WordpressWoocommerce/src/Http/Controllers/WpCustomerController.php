...
class WpCustomerController extends Controller
{
    /**
    ...
    {
        if(Auth::user()->isAbleTo('woocommerce customer manage'))
        {
            ...
            if(!empty($consumer_key ) && !empty($consumer_secret))
            {
                ...
                $ch = curl_init();
                curl_setopt($ch, CURLOPT_URL, $api_url . 'customers?per_page=' . 100);
                ...
                $wp_customers = json_decode($response, true);

                return view('wordpress-woocommerce::wpcustomer.index',compact('wp_customers'));
            }
            else
            {
