...
class FileDatatable extends DataTable
{
    ...
    {
        $rawColumns = ['file_status', 'user_id', 'is_pass_enable','file_name'];
        $dataTable = (new EloquentDataTable($query))
            ...
            ->addColumn('file_name', function (FileShare $fileShare) {
                $file_name = basename($fileShare->file_path);
                return !empty($fileShare->file_name) ? $fileShare->file_name  : $file_name;
            })
            ->addColumn('is_pass_enable', function (FileShare $fileShare) {
                return $fileShare->is_pass_enable == 1 ? __('Yes') : __('No');
            })
            ->addColumn('file_download', function (FileShare $fileShare) {
                return FileDownload::where('file_id', $fileShare->id)->get()->count();
            });
        ...
        if (\Laratrust::hasPermission('files delete') || \Laratrust::hasPermission('files edit')) {
            $dataTable->addColumn('action', function (FileShare $fileShare) {
                return view('file-sharing::files.action', compact('fileShare'));
            });
            $rawColumns[] = 'action';
        }
    ...
    {
        $query = $model::where('file_shares.created_by', creatorId())
            ->where('file_shares.workspace', getActiveWorkSpace())
            ->orderBy('file_shares.id', 'desc');

        if (!empty($request->file_status)) {
            $query->where('file_status', '=', $request->file_status);
        }
        if (!empty($request->filesharing_type)) {
            $query->where('filesharing_type', '=', $request->filesharing_type);
        }
    ...
    public function getColumns(): array
    {
        $column =  [
            ...
            Column::make('is_pass_enable')->title(__('Has password'))->searchable(false),
            Column::make('file_download')->title(__('Total Downloads')),
    ...
    {
        return 'file_shares_' . date('YmdHis');
    }
}
