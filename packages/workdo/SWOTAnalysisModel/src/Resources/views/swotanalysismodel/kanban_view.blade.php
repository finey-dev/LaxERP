                ...
                a('[data-plugin="dragula"]').each(function() {
                    ...
                    r ? dragula(n, {
                        ...
                        $("#" + target.id).parent().find('.count').text($("#" + target.id +
                            " > div").length);
                        $.ajax({
                            url: '{{ route('swotanalysismodel.order') }}',
                            type: 'POST',
                            data: {
                                creativity_id: id,
                                stage_id: stage_id,
                            ...
                            success: function(data) {
                                toastrs('Success', 'SWOT Analysis Model successfully moved',
                                    'success');
                            },
                            error: function(data) {
...
@section('page-action')
    <div class="d-flex">
        <a href="{{ route('swotanalysis-model.index') }}" class="btn btn-sm btn-primary btn-icon me-2" data-bs-toggle="tooltip"
            title="{{ __('List View') }}">
        ...
        <div class="col-12">
            ...
            <div class="row kanban-wrapper horizontal-scroll-cards pt-3" data-plugin="dragula"
                ...
                @foreach ($CharterStage as $key => $charterstage)
                    @php
                        $creativityCount = $charterstage->swotanalysismodel()->count();
                        $charters = $charterstage->swotanalysismodel();
                    @endphp
                                ...
                                @foreach ($charters as $charter)
                                    <div class="card" data-id="{{ $charter->id }}">
                                        <div
                                            ...
                                            <h5><a
                                                    href="{{ route('swotanalysis-model.show', $charter->id) }}">{{ $charter->name }}</a>
                                            </h5>
                                            <div class="card-header-right">
                                                ...
                                                <div class="dropdown-menu dropdown-menu-end">
                                                    <a href="#"
                                                        data-url="{{ route('swotanalysis-model.receipt', $charter->id) }}"
                                                    ...
                                                    @permission('SWOTAnalysisModel edit')
                                                        <a href="{{ route('swotanalysis-model.edit', $charter->id) }}"
                                        ...
                                        <div class="card-body pt-0">
                                            <div class="row my-3">
                                                <div class="align-items-center">
                                                    <a href="{{ get_file($charter->thumbnail_image) }}" target="_blank">
