...
@section('content')
    <div class="row">
        {{ Form::open(['route' => 'programs.store', 'method' => 'post', 'class' => 'w-100','class'=>'needs-validation','novalidate']) }}
        <div class="col-12">
            ...
            <div class="card">
                <div class="card-body">
                        ...
                        <div class="col-md-3">
                            <div class="form-group month">
                                <div class="btn-box">
                                    ...
                                    {{ Form::date('from_date', null, ['class' => 'form-control', 'placeholder' => 'Select Start Date', 'required' => 'required', 'min' => date('Y-m-d')]) }}
                        ...
                        <div class="col-md-3">
                            <div class="form-group month">
                                <div class="btn-box">
                                    ...
                                    {{ Form::date('to_date', null, ['class' => 'form-control form-control', 'placeholder' => 'Select End Date', 'required' => 'required', 'min' => date('Y-m-d')]) }}
                                </div>
                            </div>
                        </div>
                        <div class="col-md-6">
                    ...
                    <div class="form-group  col-md-6">
                        ...
                        <textarea class="tox-target summernote form-control" id="description" name="description" rows="3"></textarea>
                    </div>
                </div>
            </div>
        </div>
                        ...
                        <div class="col-md-12">
                            <div class="text-end d-flex all-button-box justify-content-md-end justify-content-center">
                                <a class="btn btn-sm text-light bg-primary newfield" id="add-field-btn"
                                    title="{{ __('Add Field') }}" data-bs-toggle="tooltip">
                                    ...
                                    <div class="row filter-css mt-2 form-group-container"
                                        ...
                                        <div class="col-2 form-group price-input  search-form">
                                            <div class="input-group">
                                                ...
                                                <span
                                                    class="input-group-text bg-transparent discountType">{{ '%' }}</span>
                                            </div>
                                        </div>
                                        <div class="col-auto form-group col-md-1 ml-auto text-end">
                                            <button type="button" class="btn btn-sm btn-danger  disabled delete-icon" data-toggle="tooltip" title="{{ __('Delete') }}">
        ...
        $(document).on('change', '.product_type', function() {
            ...
            $.ajax({
                ...
                success: function(data) {
                    ...
                    var product_select =
                        `<select id="${slect_ids}" class="multi-select1 choices form-control product_id item js-searchBox" name="program_details[${index}][items][]" multiple="multiple" data-placeholder="{{ __('Select multiple items Here.......') }}" required="required"></select>`;

                    $(slect_div).html(product_select);

                    // Use the correct selector to target the option
                    var option = $this.closest('.form-group-container').find('.emp_div').find(
                        '.multi-select1');
    ...
    <script>
        $(document).ready(function() {
            ...
            function addNewField(index) {
                ...
                newContainer.find('.product_type').attr('name', 'program_details[' + plusFieldIndex +
                    '][product_type]').val('product');
                newContainer.find('.from_amount').attr('name', 'program_details[' + plusFieldIndex +
                    '][from_amount]').val('');
                newContainer.find('.to_amount').attr('name', 'program_details[' + plusFieldIndex + '][to_amount]')
                    .val('');
                newContainer.find('.product_id').attr('name', 'program_details[' + plusFieldIndex + '][items][]')
                    .val('');
                newContainer.find('.discount').attr('name', 'program_details[' + plusFieldIndex + '][discount]')
                    .val('');
