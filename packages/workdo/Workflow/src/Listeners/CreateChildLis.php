...
class CreateChildLis
{
    ...
    public function handle(CreateChild $event)
    {
        ...
        if(module_is_active('Workflow'))
        {
            ...
            $workflow_module = WorkflowModule::where('submodule','New Child')->first();

            if($workflow_module)
            {
                $workflows = Workflow::where('event',$workflow_module->id)->where('workspace',$child->workspace)->where('created_by',$child->created_by)->get();
                ...
                foreach ($workflows as $key => $workflow)
                {
                    ...
                    foreach ($conditions as $key => $condition)
                    {
                        if($condition->value)
                        {
                            ...
                            if(!empty($workflow_module_field))
                            {
                                ...
                                {
                                    $status = $symbolToOperator[$symbol]($request->dob,$condition->value);
                                }
                    ...
                    if($status == true)
                    {
                        ...
                        WorkflowUtility::call_do_this($workflow->id,$child);
                    }
                }
            }
