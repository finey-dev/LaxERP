...
class CreateUserLis
{
    ...
    public function handle(CreateUser $event)
    {
        if(module_is_active('Workflow'))
        {
            ...
            $workflow_module = WorkflowModule::where('submodule','New Users')->first();

            if($workflow_module)
            {
                $workflows = Workflow::where('event',$workflow_module->id)->where('workspace',$user->workspace_id)->where('created_by',$user->created_by)->get();
                ...
                foreach ($workflows as $key => $workflow)
                {
                    ...
                    if($status == true)
                    {
                        WorkflowUtility::call_do_this($workflow->id,$user);
