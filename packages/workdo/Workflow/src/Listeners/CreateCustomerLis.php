...
class CreateCustomerLis
{
    ...
    public function handle(CreateCustomer $event)
    {
        if(module_is_active('Workflow'))
        {
            ...
            $workflow_module = WorkflowModule::where('submodule','New Customer')->first();
            if($workflow_module)
            {
                $workflows = Workflow::where('event',$workflow_module->id)->where('workspace',$customer->workspace)->where('created_by',$customer->created_by)->get();
                ...
                foreach ($workflows as $key => $workflow)
                {
                    ...
                    foreach ($conditions as $key => $condition)
                    {
                        if($condition->value)
                        {
                            ...
                            if(!empty($workflow_module_field))
                            {
                                ...
                                else if($workflow_module_field->field_name == 'Tax Number')
                                {
                                    $status = $symbolToOperator[$symbol]($request->tax_number,$condition->value);
                                }
                    ...
                    if($status == true)
                    {
                        WorkflowUtility::call_do_this($workflow->id,$customer);
                    }
                }
            }
