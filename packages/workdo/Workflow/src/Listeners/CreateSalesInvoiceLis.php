...
class CreateSalesInvoiceLis
{
    ...
    public function handle(CreateSalesInvoice $event)
    {
        ...
        if(module_is_active('Workflow'))
        {
            ...
            $workflow_module = WorkflowModule::where('submodule','New Sales Invoice')->first();
                    ...
                    foreach ($conditions as $key => $condition)
                    {
                        if($condition->value)
                        {
                            ...
                            if(!empty($workflow_module_field))
                            {
                                ...
                                if($workflow_module_field->field_name == 'Quote')
                                {
                                    $status = $symbolToOperator[$symbol]($request->quote,$condition->value);
                                }
                                else if($workflow_module_field->field_name == 'Opportunity')
                                {
                                    $status = $symbolToOperator[$symbol]($request->opportunity,$condition->value);
                                }
