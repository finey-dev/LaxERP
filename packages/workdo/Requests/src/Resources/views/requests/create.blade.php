{{ Form::open(['url' => 'requests' ,'class'=>'needs-validation','novalidate']) }}
<div class="modal-body">
    <div class="row">
        <div class="col-md-6">
            <div class="form-group">
                {{ Form::label('name', __('Name'), ['class' => 'form-label']) }}<x-required></x-required>
                {{ Form::text('name', null, ['class' => 'form-control', 'placeholder' => __('Enter Request'), 'required' => 'required']) }}
                @error('name')
        ...
        <div class="col-md-6">
            <div class="form-group">
                ...
                {{ Form::select('category', $requestscategory, null, ['class' => 'form-control category', 'placeholder' => __('Select Category'), 'required' => 'required']) }}
            </div>
        </div>

        <div class="col-md-6 subcategorys" style="display: none;">
            <div class="form-group">
                {{ Form::label('subcategory', __('SubCategory'), ['class' => 'form-label']) }}<x-required></x-required>
                {{ Form::select('subcategory', $requestssubcategory, null, ['class' => 'form-control subcategory', 'placeholder' => __('Select SubCategory'), 'required' => 'required']) }}
            </div>
        </div>
    </div>
        ...
        <div class="form-group mt-4 mb-3">
            ...
            <div class="form-check form-switch custom-switch-v1">
                ...
                <input type="checkbox" class="form-check-input input-primary" name="active" id="active"
                    {{ !empty($Requests->active) && $Requests->active == 'on' ? 'checked' : '' }}>
    ...
    <div class="row modal-body-card">
        ...
        <div class="col-lg-4 col-sm-6 business-view-card {{ $loop->first ? 'selected-theme' : '' }}">
            <label for="Formlayout1">
                <input type="hidden" name="layouts" class="themefile1">
                <div class="business-view-inner">
                    ...
                    <div class="mt-3">
                        <h6>{{ __('Form Layout ' . ($loop->index + 1)) }}</h6>
                        ...
                        <div class="d-flex align-items-center business-color-input" id="{{ $key }}">
                            ...
                            <label class="colorinput">
                                <input type="radio" name="theme_color" id="color1-{{ $key }}"
                                    ...
                                    class="colorinput-input" @if($key == 'Formlayout1' && $val['theme_name'] == 'Formlayout1-v1') checked @endif>
