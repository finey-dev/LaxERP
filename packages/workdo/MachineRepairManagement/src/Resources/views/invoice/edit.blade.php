...
@section('page-breadcrumb')
    {{ __('Diagnosis Edit') }}  {{--Machine Repair Invoice--}}
@endsection
@push('scripts')
    ...
    <script>
        ...
        $(document).on('click', '[data-repeater-delete]', function ()
        {
            ...
            $.ajax({
                url: '{{route('machine.invoice.product.destroy')}}',
                type: 'POST',
                headers: {
...
@section('content')
    <div class="row">
        {{ Form::model($invoice, array('route' => array('machine-repair-invoice.update', $invoice->id), 'method' => 'PUT','class'=>'w-100', 'enctype' => 'multipart/form-data')) }}
                                    ...
                                    {{ Form::label('request_id', __('Machine Repair Request'), ['class' => 'form-label']) }}
                                        <select class="form-control staff " name="request_id" id="customer" data-url="{{ route('machine.invoice.request') }}" tabindex="-1" required
                                            ...
                                            @foreach ($requests as $request)
                                                <option value="{{ $request }}" @if ($invoice->request_id == $request) selected @endif>
                            ...
                            <div class="row">
                                ...
                                <div class="col-md-6">
                                    <div class="form-group">
                                        ...
                                        <div class="form-icon-user input-group">
                                            <input type="number" class="form-control service_charge" name="service_charge" id="service_charge" placeholder="{{__('Enter Service Charge')}}" value="{{ $invoice->service_charge }}" required> {{-- value="{{ $service_charge }}" --}}
                                ...
                                <div class="col-md-6">
                                    <div class="form-group">
                                        ...
                                        <div class="form-icon-user">
                                            <input type="number" class="form-control" name="estimated_time" id="estimated_time" placeholder="{{__('Enter Estimated Repair Time')}}" value="{{ $invoice->estimated_time }}" required> {{-- value="{{ $estimated_time }}" --}}
        ...
        <div class="modal-footer mb-3">
            <input type="button" value="{{__('Cancel')}}" onclick="location.href = '{{route('machine-repair-invoice.index')}}';" class="btn btn-light mx-3">
