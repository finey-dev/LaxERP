...
{{ Form::open(array('url' => 'contract','enctype'=>'multipart/form-data','class'=>'needs-validation','novalidate')) }}
    ...
    {{ Form::hidden('contract_type','contract',null) }}
        ...
        @if (\Auth::user()->type == 'company')
            <div class="col-md-6 form-group">
                ...
                {{ Form::select('user_id',$user,null, array('class' => 'form-control','id'=>'user_id','required'=>'required','placeholder' => __('Select User'))) }}
            </div>
        @else
        ...
        <div class="col-md-6 form-group">
            ...
            {{ Form::number('value', '', array('class' => 'form-control','required'=>'required','min' => '1','placeholder'=>__('Enter Amount'))) }}
        </div>
        <div class="col-md-4">
        ...
        <div class="col-md-4">
            <div class="form-group">
                ...
                {!!Form::date('start_date', date('Y-m-d'),array('class' => 'form-control','placeholder' => __('Start Date'),'required'=>'required')) !!}
            </div>
        </div>
        <div class="col-md-4">
            <div class="form-group">
                {{Form::label('end_date',__('End Date'),['class'=>'form-label']) }}<x-required></x-required>
                {!!Form::date('end_date', date('Y-m-d'),array('class' => 'form-control','required'=>'required','placeholder' => __('End Date'))) !!}
            </div>
        </div>
        <div class="col-md-12">
            <div class="form-group">
                {{ Form::label('notes', __('Description'),['class'=>'form-label']) }}
                {{ Form::textarea('notes', '', array('class' => 'form-control','placeholder'=>__('Enter Description'),'rows' => '3')) }}
            </div>
        </div>
