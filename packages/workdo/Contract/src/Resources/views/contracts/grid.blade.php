...
@section('page-action')
    <div class="d-flex">
        @stack('addButtonHook')
        <a href="{{ route('contract.index') }}" class="btn btn-sm btn-primary btn-icon me-2" data-bs-toggle="tooltip"
            title="{{ __('List View') }}">
...
@section('content')
    ...
    <div class="row row-gap-2 mb-4">
        @foreach ($contracts as $contract)
            <div class="col-xxl-3 col-xl-4 col-md-6">
                <div class="card grid-card user-card">
                    <div class="card-header border border-bottom p-3 d-flex justify-content-between h-100 align-items-start gap-1">
                        <div class="user-img-wrp d-flex align-items-center">
                            <div class="user-image rounded border-2 border border-primary">
                                <img alt="user-image" class="h-100 w-100"
                                    @if (!empty($contract->avatar)) src="{{ !empty($contract->avatar) ? get_file($contract->avatar) : asset(url('./assets/img/clients/160x160/img-1.png')) }}" @else  avatar="{{ $contract->subject }}" @endif>
                            </div>
                            <div class="user-content">
                                <h4 class="mb-2 text-break"> <a href="{{ route('contract.show', $contract->id) }}">
                                        {{ Workdo\Contract\Entities\Contract::contractNumberFormat($contract->contract_id) }}
                                    </a></h4>
                                <a href="{{ route('contract.show', $contract->id) }}"
                                    data-title="{{ __('Contact Details') }}" class="action-item text-primary mt-2"><span
                                        class="text-dark text-md">
                                        {{ ucfirst($contract->subject) }}</span>
                                </a>
                            </div>
                        </div>
                                    ...
                                    @if (Laratrust::hasPermission('contract create') || Laratrust::hasPermission('contract show') || Laratrust::hasPermission('contract edit') || Laratrust::hasPermission('contract delete') || Laratrust::hasPermission('contract template create'))
                                        ...
                                        @if (module_is_active('ContractTemplate'))
                                            @permission('contract template create')
                                                <a class="dropdown-item" data-ajax-popup="true"
                                                    data-size="md" data-title="{{ __('Save As Contract Template') }}"
                                                    data-url="{{ route('contract-template.create',['contract_id'=> $contract->id,'type'=>'template']) }}">
                                                    <i class="ti ti-bookmark me-1"></i> <span>{{ __('Save as template') }}</span>
                                                </a>
                                            @endpermission
                                        @endif
                                        ...
                                        @permission('contract create')
                                            <a data-url="{{ route('contracts.copy',$contract->id) }}" data-size="lg" data-ajax-popup="true"  class="dropdown-item"
                                                ...
                                                data-title="{{ __('Duplicate Contract') }}"><i class="ti ti-copy me-1"></i>
                                                {{ __('Duplicate') }}</a>
                                        @endpermission
                                        @permission('contract show')
                                            <a href="{{ route('contract.show', $contract->id) }}" class="dropdown-item"
                                                ...
                                                data-title="{{ __('Contract Details') }}"><i class="ti ti-eye me-1"></i>
                                                {{ __('View') }}</a>
                                        @endpermission
                                        ...
                                        @permission('contract edit')
                                            <a data-url="{{ route('contract.edit', $contract->id) }}" data-size="lg" data-ajax-popup="true"  class="dropdown-item"
                                                ...
                                                data-title="{{ __('Edit Contract') }}"><i class="ti ti-pencil me-1"></i>
                                                {{ __('Edit') }}</a>
                                        @endpermission
                    ...
                    <div class="card-body  text-center p-3">
                        <div class="bottom-icons d-flex flex-wrap align-items-center justify-content-between">
                            <div class="edit-btn-wrp d-flex flex-wrap align-items-center">
                                @if (Laratrust::hasPermission('contract show') || Laratrust::hasPermission('contract edit') || Laratrust::hasPermission('contract delete'))
                                    @permission('contract show')
                                        <a class="btn btn-sm border d-flex" href="{{ route('contract.show', $contract->id) }}"
                                    ...
                                    @permission('contract edit')
                                        <a href="#!" data-url="{{ route('contract.edit', $contract->id) }}"
                                            data-ajax-popup="true" data-size="md" data-title="{{ __('Edit Contract') }}"
                                            data-bs-original-title="{{ __('Edit') }}" data-bs-toggle="tooltip"
                                            data-bs-placement="top" class="btn btn-sm border d-flex">
                                    ...
                                    @permission('contract delete')
                                        {{ Form::open(['route' => ['contract.destroy', $contract->id], 'class' => 'm-0']) }}
                                        @method('DELETE')
                                        <a href="#!" aria-label="Delete" data-confirm="{{ __('Are You Sure?') }}"
                                            data-text="{{ __('This action can not be undone. Do you want to continue?') }}"
                                            data-confirm-yes="delete-form-{{ $contract->id }}" data-bs-toggle="tooltip"
                                            data-bs-placement="top" data-bs-original-title="{{ __('Delete') }}"
                                            class="btn btn-sm border bs-pass-para show_confirm d-flex">
                                            <svg xmlns="http://www.w3.org/2000/svg" width="16" height="16"
                                                viewBox="0 0 16 16" fill="none">
        ...
        @permission('contract create')
            <div class="col-xxl-3 col-xl-4 col-md-6">
                <a data-url="{{ route('contract.create') }}" class="btn-addnew-project border-primary"
                    data-ajax-popup="true" data-size="lg" data-title="{{ __('Create Contract') }}">
                    <div class="bg-primary proj-add-icon">
                        <i class="ti ti-plus my-2"></i>
                    </div>
                    <h6 class="my-2 text-center">{{ __('Contract') }}</h6>
                    <p class="text-muted text-center">{{ __('Click here to add New Contract') }}</p>
                </a>
            </div>
        @endpermission
    </div>
    {!! $contracts->links('vendor.pagination.global-pagination') !!}
@endsection
