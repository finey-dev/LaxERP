<div class="bg-none card-box">
    {{ Form::open(['route' => 'repair-warranty.store', 'class' => 'needs-validation', 'novalidate']) }}
        ...
        <div class="row">
            ...
            <div class="col-md-12">
                <div class="form-group">
                    ...
                    @if (empty($repair_parts->count()))
                        <div class="text-xs p-1">
                            {{ __('Please add repair parts for warranty. ') }}
                            <a href="{{ route('repair.request.index') }}">
                                <b>{{ __('Add repair parts') }}</b>
                            </a>
                        </div>
                    @endif
                </div>
            ...
            if (repairOrderId) {
                $.ajax({
                    ...
                    success: function(data) {
                        $('#part_id').empty();
                        $('#part_id').append('<option value="">' + 'Select Part' +
                            '</option>');

                        $.each(data, function(key, value) {
                            $('#part_id').append('<option value="' + key + '">' +
                                value + '</option>');
                        });
                    }
                });
