...
@section('content')
<div class="row">
    {{ Form::model($repair_order_request, array('route' => array('repair.parts.update', $repair_order_request->id), 'method' => 'PUT')) }}
    <input type="hidden" name="type" id="type" value="{{$type}}">
    <div class="col-12">
        <h5 class="h4 d-inline-block font-weight-400 mb-4">{{ __('Repair Product Detail') }}</h5>
        <div class="card">
            <div class="card-body">
                <div class="container">
                    <div class="p-sm-4 p-3 invoice-billed">
                        <div class="row row-gap">
                            ...
                            <div class="col-lg-4 col-sm-6 text-end">
                                ...
                                <span class="badge fix_badge f-12 p-2 d-inline-block bg-primary">{{__('Start Repairing')}}</span>
                                @elseif($repair_order_request->status == 2)
                                <span class="badge fix_badge f-12 p-2 d-inline-block bg-primary">{{__('End Repairing')}}</span>
                                @elseif($repair_order_request->status == 3)
                                <span class="badge fix_badge f-12 p-2 d-inline-block bg-primary">{{__('Start Testing')}}</span>
                                @elseif($repair_order_request->status == 4)
                                <span class="badge fix_badge f-12 p-2 d-inline-block bg-primary">{{__('End Testing')}}</span>
                                @elseif($repair_order_request->status == 5)
                                <span class="badge fix_badge f-12 p-2 d-inline-block bg-warning">{{__('irreparable')}}</span>
                                @elseif($repair_order_request->status == 6)
                                <span class="badge fix_badge f-12 p-2 d-inline-block bg-danger">{{__('Cnacel')}}</span>
                                @elseif($repair_order_request->status == 7)
                                <span class="badge fix_badge f-12 p-2 d-inline-block bg-info">{{__('Invoice created')}}</span>
                                @endif
                            </div>
                        </div>
                    </div>
    ...
    <div class="col-12 section_div">
        ...
        <div class="card repeater" data-value='{!! json_encode($repair_order_request->repairParts) !!}'>
                    ...
                    <table class="table  mb-0 table-custom-style" data-repeater-list="items" id="sortable-table">
                        ...
                        <tbody class="ui-sortable" data-repeater-item>
                            <tr>
                                ...
                                <td>
                                    <div class="form-group price-input input-group search-form m-0" style="width: 160px">
    ...
    <div class="modal-footer">
        <input type="button" value="{{ __('Back') }}" onclick="location.href ='{{ !$type ? route('repair.request.index') : route("repair.request.invoice.show", [\Crypt::encrypt($type)]) }}'" class="btn btn-light ">
        <input type="submit" value="{{ __('Save Changes') }}" class="btn  btn-primary mx-3 submit">
    </div>
    {{ Form::close() }}
</div>
@endsection
...
<script>
    var repair_id = '{{ $repair_order_request->id }}';
            ...
            success: function(data) {
                ...
                $.ajax({
                    ...
                    success: function(data) {
                        ...
                        for (var i = 0; i < item.taxes.length; i++) {
                            taxes +=
                                '<span class="badge bg-primary p-2 px-3 mt-1 mr-1" style="margin-right:5px;">' +
                                item.taxes[i].name + ' ' + '(' + item.taxes[i].rate + '%)' +
                                '</span>';
                            tax.push(item.taxes[i].id);
                            totalItemTaxRate += parseFloat(item.taxes[i].rate);
    ...
    $(document).on('click', '[data-repeater-delete]', function() {
            ...
            $.ajax({
                url: "{{ route('repair.parts.destroy') }}",
                type: 'POST',
                data: {
                    _token: "{{ csrf_token() }}",
                    'id': id
                ...
                error: function(data) {
                    // toastrs('Error', "{{ __('Deleted') }}", 'error');
                },
            });
        });
</script>
