...
@section('content')
    <div class="row">
        {{ Form::model($quotation, ['route' => ['quotation.update', $quotation->id], 'method' => 'PUT', 'class' => 'w-100', 'enctype' => 'multipart/form-data', 'class' => 'needs-validation', 'novalidate']) }}

        @if ($quotation->quotation_module == 'account')
            <input type="hidden" name="quotation_type" id="quotation_type" value="product">
        @elseif ($quotation->quotation_module == 'taskly')
            <input type="hidden" name="quotation_type" id="quotation_type" value="project">
        @elseif ($quotation->quotation_module == 'cmms')
            <input type="hidden" name="quotation_type" id="quotation_type" value="parts">
        @endif
                        ...
                        <div class="col-xs-12 col-sm-12 col-md-6 col-lg-6">
                            <div class="row" id="customer-box">
                                <div class="form-group col-md-6 " id="account-box">
                                    ...
                                    <select
                                        ...
                                        @if (module_is_active('Account'))
                                            <option value="Accounting" @if ($quotation->account_type == 'Accounting') selected @endif>
                                                {{ __('Accounting') }}</option>
                                        @endif
                                        @if (module_is_active('Taskly'))
                                            <option value="Projects" @if ($quotation->account_type == 'Projects') selected @endif>
                                                {{ __('Projects') }}</option>
                                        @endif
                                        @if (module_is_active('CMMS'))
                                            <option value="Parts" @if ($quotation->account_type == 'Parts') selected @endif>
                                                {{ __('CMMS') }}</option>
                                        @endif
                                    </select>
                                ...
                                <div class="form-group col-md-6">
                                    ...
                                    {{ Form::select('customer_id', $customers, null, ['class' => 'form-control', 'id' => 'customer', 'data-url' => route('proposal.customer'), 'required' => 'required', 'placeholder' => __('Select Customer')]) }}
                            ...
                            <div class="row">
                                ...
                                <div class="col-md-12">
                                    <div class="row">
                                        <div class="col-md-6 form-group">
                                            {{ Form::label('quotation', __('Quotation Type'), ['class' => 'form-label']) }}<x-required></x-required>
                                            <select
                                                class="form-control quotation {{ !empty($errors->first('quotation')) ? 'is-invalid' : '' }}"
                                                name="quotation" required="" id="quotation">
                                                <option value="">{{ __('Select Quotation Type') }}</option>
                                                <option value="invoice" id="invoice" class="quotation-type"
                                                    @if ($quotation->quotation == 'invoice') selected @endif>
                                                    {{ __('Invoice') }}</option>
                                                @if (module_is_active('Pos'))
                                                    <option value="pos" id="pos" class="quotation-type"
                                                        @if ($quotation->quotation == 'pos') selected @endif>
                                                        {{ __('Pos') }}</option>
                                                @endif
                                            </select>
                                        </div>
                                        <div class="form-group col-md-6">
                                            ...
                                            <select
                                                ...
                                                name="quotation_type_radio" required="" id="billing_type">
                                                <option value="">{{ __('Select Billing Type') }}</option>
                                                <option value="product" id="product"
                                                    @if ($quotation->quotation_module == 'account') selected @endif>
                                                    {{ __('Item Wise') }}</option>
                                                <option class="{{ $quotation->quotation == 'pos' ? 'd-none' : '' }} "
                                                    value="project" id="project1"
                                                    @if ($quotation->quotation_module == 'taskly') selected @endif>
                                                    {{ __('Project Wise') }}</option>
                                                <option value="parts" id="parts"
                                                    @if ($quotation->quotation_module == 'cmms') selected @endif>
                                ...
                                <div class="col-md-6">
                                    <div class="form-group">
                                        ...
                                        {{ Form::select('warehouse_id', $warehouse, null, ['class' => 'form-control select warehouse', 'required' => 'required']) }}
                                    </div>
                                </div>
                                <div class="col-md-6">
                                    <div class="form-group">
                                ...
                                <div class="col-md-6">
                                    <div class="form-group">
                                        ...
                                        <label for="quotation_template" class="form-label">{{ __('Template') }}</label><x-required></x-required>
                                        <select class="form-control" name="quotation_template" id="quotation_template" required="">
                                            <option value="">{{ __('Select Template') }}</option>
                                            @foreach (templateData()['templates'] as $key => $template)
                                                <option value="{{ $key }}"
                                                    {{ $quotation->quotation_template == $key ? 'selected' : '' }}>
                                                    {{ $template }}</option>
                                            @endforeach
                                        </select>
                                    </div>
                                ...
                                <div class="col-md-6">
                                    <div class="form-group quotation_div">
                                        @if (module_is_active('Taskly'))
                                            ...
                                            {{ Form::select('project', $projects, $quotation->category_id, ['class' => 'form-control project_id', 'required' => 'required']) }}
                                        @endif
                                    </div>
                                </div>
                                @if (module_is_active('Taskly'))
                                    <div
                                        ...
                                        <div class="form-group">
                                            ...
                                            {{ Form::select('tax_project[]', $taxs, !empty($quotation->items->first()->tax) ? explode(',', $quotation->items->first()->tax) : null, ['class' => 'form-control get_tax multi-select choices', 'multiple' => 'multiple', 'id' => 'tax_project']) }}
                                        </div>
                                    </div>
                                @endif
                            </div>
        ...
        <div class="modal-footer mb-3">
            <input type="button" value="{{ __('Cancel') }}"
                onclick="location.href = '{{ route('quotation.index') }}';" class="btn btn-light me-2">
...
@push('scripts')
    ...
    <script>
        $(document).ready(function() {
            $("input[name='warehouse_id']:checked").trigger('change');
        });
            ...
            $.ajax({
                ...
                success: function(data) {
                    ...
                    if (data.error) {
                        ...
                        '{{ __('The product has been deleted successfully') }}') {
    ...
    @if (module_is_active('Account') || module_is_active('CMMS'))
        <script>
            ...
            function items(data) {
                ...
                var in_type = $('#quotation_type').val();
                if (in_type == 'product' || in_type == 'parts') {
                        ...
                        success: function(data) {
                            ...
                            if (item.taxes == 0) {
                                ...
                                for (var i = 0; i < item.taxes.length; i++) {
                                    taxes +=
                                        '<span class="badge bg-primary p-2 px-3  mt-1 me-1 product_tax">' +
    ...
    <script>
        function SectionGet(type = 'product', project_id = "0", title = 'Project', warehouse = "0") {

            var acction = $("#acction_type").val();
            $.ajax({
                type: 'post',
                url: "{{ route('quotation.section.type') }}",
                data: {
                    _token: "{{ csrf_token() }}",
                    type: type,
        ...
        function SectionGet(type = 'parts', project_id = "0", title = 'Project', warehouse = "0") {
    ...
    <script>
        $(document).on('change', ".warehouse", function() {
            ...
            if (val == 'product') {
                ...
                var label =
                    ` {{ Form::label('project', __('Project'), ['class' => 'form-label']) }} {{ Form::select('project', $projects, $quotation->category_id, ['class' => 'form-control', 'required' => 'required']) }}`
                $(".quotation_div").append(label);
                $("#quotation_type").val('project');
                var project_id = $("#project").val();

                if (quotation_module == 'taskly') {
                    $("#acction_type").val('edit');
                } else {
                    $("#acction_type").val('create');
                }

                SectionGet(val, project_id, 'Project', warehouse);
            } else if (val == 'parts') {

                $(".discount_apply_div").removeClass('d-none');
                $(".tax_project_div").addClass('d-none');
                $(".discount_project_div").addClass('d-none');
                $("#quotation_type").val('parts');

                if (quotation_module == 'cmms') {
                    $("#acction_type").val('edit');
                } else {
                    $("#acction_type").val('create');
                }
                SectionGet(val, 0, 'Parts', warehouse);
            }
            choices();
        });
    </script>
    ...
    @if (module_is_active('Account') && $quotation->quotation_module == 'account')
        <script>
            $(document).ready(function() {
    ...
    @elseif (module_is_active('Taskly') && $quotation->quotation_module == 'taskly')
        <script>
            $(document).ready(function() {
                $('.tax_project_div').removeClass('d-none');
    ...
    @elseif (module_is_active('CMMS') && $quotation->quotation_module == 'cmms')
        <script>
            $(document).ready(function() {
                $('.project_id').addClass('d-none');
                $('.tax_project_div').addClass('d-none');
    ...
    <script>
        $(document).ready(function() {
            $('#account_type').on('change', function() {
                var type = $(this).val();
                if (type == 'Accounting') {
                    $('#invoice').removeClass('d-none').show();
                    $('#pos').removeClass('d-none').show();
