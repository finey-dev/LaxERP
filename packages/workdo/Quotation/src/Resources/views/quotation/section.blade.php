    ...
    if ($(selector + " .repeater").length) {
        ...
        var $repeater = $(selector + ' .repeater').repeater({
            ...
            show: function() {
                ...
                var items = $(selector + ' .repeater .item');
                items.each(function(index) {

                    if (index === 0) {
                        console.log();

                        $(this).parent().parent('.quotation-div').find('.delete-btn').hide();
                    } else {
                        $(this).parent().parent('.quotation-div').find('.delete-btn').show();
                    }
                });
        ...
        $.ajax({
            ...
            success: function(data) {
                ...
                var product_select = `<select class="form-control product_id item js-searchBox" name="${itemSelect}"
                                        placeholder="Select Item" data-url="{{ route('quotation.product') }}" required = 'required'>
                                        </select>`;
...
@if ($type == 'product')
    ...
    <div class="card repeater" @if ($acction == 'edit') data-value='{!! json_encode($quotation->items) !!}' @endif>
                        ...
                        <tr class="quotation-div">
                            ...
                            <td width="25%" class="form-group pt-0 product_div">
                                <select name="item" class="form-control product_id item  js-searchBox"
                                    data-url="{{ route('quotation.product') }}" required>
