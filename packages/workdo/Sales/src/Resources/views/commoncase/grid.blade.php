...
@section('page-action')
    <div class="d-flex">
        @stack('addButtonHook')
        <a href="{{ route('commoncases.index') }}" class="btn btn-sm btn-primary btn-icon me-2" data-bs-toggle="tooltip"
            title="{{ __('List View') }}">
            ...
            <div class="col-xxl-3 col-xl-4 col-md-6">
                <div class="card grid-card user-card">
                    <div class="card-header border border-bottom p-3 h-100 d-flex gap-1 justify-content-between">
                        <div class="user-img-wrp d-flex align-items-start w-100">
                            <div class="user-image rounded border-2 border border-primary">
                                <img src="{{ check_file($commonCase->attachments) ? get_file($commonCase->attachments) : get_file('uploads/users-avatar/avatar.png') }}"
                                    alt="user-image" class="h-100 w-100">
                            </div>
                            <div class="user-content">
                                <h4 class="mb-2"> <a href="#"
                                        data-url="{{ route('commoncases.show', $commonCase->id) }}" data-ajax-popup="true"
                                        data-title="{{ __('Details') }}" class=" text-dark">{{ $commonCase->name ?? '-' }}
                                    </a></h4>
                                <span
                                    class="text-dark text-md">{{ !empty($commonCase->accounts) ? $commonCase->accounts->name : '-' }}</span>
                            </div>
                                    ...
                                    <div class="dropdown-menu dropdown-menu-end pointer">
                                        ...
                                        @permission('case show')
                                            <a href="#" data-url="{{ route('commoncases.show', $commonCase->id) }}"
                                                data-ajax-popup="true" data-bs-whatever="{{ __('Common case Details') }}"
                                                data-title="{{ __('Case Details') }}" data-bs-original-title="{{ __('Details') }}"
                                                data-bs-toggle="tooltip" data-bs-placement="top" class="dropdown-item"><i class="ti ti-eye me-1"></i> {{ __('View') }}
                                            </a>
                                        @endpermission
                                        @permission('case edit')
                                            <a href="{{ route('commoncases.edit', $commonCase->id) }}" data-size="md"
                                                data-title="{{ __('Edit Common case ') }}"
                                                data-bs-original-title="{{ __('Edit') }}" data-bs-toggle="tooltip"
                                                data-bs-placement="top" class="dropdown-item"> <i class="ti ti-pencil me-1"></i> {{ __('Edit') }}
                                            </a>
                                        @endpermission
                                        @permission('case delete')
                                            {{ Form::open(['route' => ['commoncases.destroy', $commonCase->id], 'class' => 'm-0']) }}
                                            @method('DELETE')
                                            <a href="#!" aria-label="Delete" data-confirm="{{ __('Are You Sure?') }}"
                                                data-text="{{ __('This action can not be undone. Do you want to continue?') }}"
                                                data-confirm-yes="delete-form-{{ $commonCase->id }}" data-bs-toggle="tooltip"
                                                data-bs-placement="top" data-bs-original-title="{{ __('Delete') }}"
                                                class=" dropdown-item show_confirm"><span class="text-danger"><i class="ti ti-trash me-1"></i> {{ __('Delete') }}</span>
                        ...
                        <div class="bottom-icons d-flex flex-wrap align-items-center justify-content-between">
                            <div class="edit-btn-wrp d-flex flex-wrap align-items-center">
                                @permission('case show')
                                    <a href="#" data-url="{{ route('commoncases.show', $commonCase->id) }}"
                                        data-ajax-popup="true" data-bs-whatever="{{ __('Common case Details') }}"
                                        data-title="{{ __('Case Details') }}" data-bs-original-title="{{ __('View') }}"
                            ...
                            @if ($commonCase->status == 0)
                                <span
                                    class="badge bg-success p-2 px-3 ">{{ __(Workdo\Sales\Entities\CommonCase::$status[$commonCase->status]) }}</span>
                            @elseif($commonCase->status == 1)
                                <span
                                    class="badge bg-info p-2 px-3">{{ __(Workdo\Sales\Entities\CommonCase::$status[$commonCase->status]) }}</span>
                            @elseif($commonCase->status == 2)
                                <span
                                    class="badge bg-warning p-2 px-3">{{ __(Workdo\Sales\Entities\CommonCase::$status[$commonCase->status]) }}</span>
                            @elseif($commonCase->status == 3)
                                <span
                                    class="badge bg-danger p-2 px-3">{{ __(Workdo\Sales\Entities\CommonCase::$status[$commonCase->status]) }}</span>
            ...
            @permission('case create')
                <div class="col-xxl-3 col-xl-4 col-md-6">
                    <a href="#" class="btn-addnew-project border-primary p-4" data-ajax-popup="true" data-size="md"
                        ...
                        data-url="{{ route('commoncases.create', ['commoncases', 0]) }}">
                        <div class="bg-primary proj-add-icon">
                            <i class="ti ti-plus my-2"></i>
                        </div>
                        <h6 class="my-2 text-center">{{ __('Create case ') }}</h6>
                        <p class="text-muted text-center mb-0">{{ __('Click here to add New case ') }}</p>
                    </a>
                </div>
            @endpermission
        @endauth
        {!! $commonCases->links('vendor.pagination.global-pagination') !!}
    </div>
@endsection
