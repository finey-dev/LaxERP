...
<head>
    ...
    <title>
        {{ Workdo\Sales\Entities\SalesInvoice::invoiceNumberFormat($invoice->invoice_id, $invoice->created_by, $invoice->workspace) }}
        |
        {{ !empty(company_setting('title_text', $invoice->created_by, $invoice->workspacey)) ? company_setting('title_text', $invoice->created_by, $invoice->workspacey) : (!empty(admin_setting('title_text')) ? admin_setting('title_text') : 'WorkDo') }}
    </title>
    <link
    ...
    <div class="invoice-preview-main" id="boxes" style="border-right:40px solid var(--theme-color);">
        ...
        <div class="invoice-body" style="padding-right: 0;">
                    ...
                    @if (isset($invoice->items) && count($invoice->items) > 0)
                        @foreach ($invoice->items as $key => $item)
                            <tr>
                                ...
                                <td>{{ currency_format_with_sym($item->price * $item->quantity, $invoice->created_by, $invoice->workspace) }}
                                </td>
            ...
            <div class="invoice-footer">
                <p> {{ $settings['salesinvoice_footer_title'] }}</p>
                <h6>{{ $settings['salesinvoice_footer_notes'] }}</h6>
            </div>
        </div>
    </div>
    @if (!isset($preview))
