...
<head>
    ...
    <title>{{Workdo\Sales\Entities\SalesInvoice::invoiceNumberFormat($invoice->invoice_id,$invoice->created_by,$invoice->workspace)}} | {{ !empty(company_setting('title_text',$invoice->created_by,$invoice->workspace)) ? company_setting('title_text',$invoice->created_by,$invoice->workspace) : (!empty(admin_setting('title_text')) ? admin_setting('title_text') :'WorkDo') }}</title>
            ...
            <table>
                <tbody>
                    <tr>
                        ...
                        <td class="text-right">
                            <h3 style="text-transform: uppercase; font-size: 40px; font-weight: bold; ">{{ __('Sales Invoice')}}</h3>
                        </td>
                    </tr>
                </tbody>
            </table>
                        ...
                        <td style="width: 60%;">
                            <table class="no-space">
                                <tbody>
                                    <tr>
                                        <td>{{ __('Number: ')}}</td>
                                        <td class="text-right">{{Workdo\Sales\Entities\SalesInvoice::invoiceNumberFormat($invoice->invoice_id,$invoice->created_by,$invoice->workspace)}}</td>
                                    </tr>
                                    <tr>
                                        <td>{{ __('Issue Date:')}}</td>
                                    ...
                                    @if(!empty($customFields) && count($invoice->customField)>0)
                                        ...
                                        @endforeach
                                    @endif
                                    @if($settings['sales_invoice_qr_display'] == 'on')
                                    <tr>
                                        <td colspan="2">
                                            <div class="view-qrcode">
                                                <p> {!! DNS2D::getBarcodeHTML(route('pay.salesinvoice',\Illuminate\Support\Facades\Crypt::encrypt($invoice->invoice_id)), "QRCODE",2,2) !!}
                                            </div>
                                        </td>
                                    </tr>
                                    @endif
                    ...
                    @if(isset($invoice->items) && count($invoice->items) > 0)
                        @foreach($invoice->items as $key => $item)
                            <tr>
                                ...
                                <td>{{currency_format_with_sym($item->discount,$invoice->created_by,$invoice->workspace)}}</td>
                                <td>
