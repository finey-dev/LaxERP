...
@section('page-action')
    <div class="d-flex">
        ...
        <a href="{{ route('salesdocument.index') }}" class="btn btn-sm btn-primary btn-icon me-2" data-bs-toggle="tooltip"
            title="{{ __('List View') }}">
                    ...
                    <div class="card-header border border-bottom p-3 h-100">
                        <div class="user-img-wrp d-flex align-items-center">
                            <div class="user-image rounded border-2 border border-primary">
                                <img @if (!empty($document->avatar)) src="{{ !empty($document->avatar) ? asset(Storage::url('upload/profile/' . $document->avatar)) : asset(url('./assets/img/clients/160x160/img-1.png')) }}" @else  avatar="{{ $document->name }}" @endif
                                    alt="user-image" class="h-100 w-100">
                            </div>
                            <div class="user-content">
                                @if (\Laratrust::hasPermission('salesdocument show'))
                                    <h4 class="mb-2 text-break"><a class="text-primary" data-url="{{ route('salesdocument.show', $document->id) }}"
                                            href="#" data-ajax-popup="true" data-title="{{ __('Sales Document Details') }}">{{ ucfirst($document->name) }}</a>
                                    </h4>
                                @else
                                    <h4 class="mb-2">{{ ucfirst($document->name) }}</h4>
                                @endif
                            </div>
                        </div>
                    </div>
                            ...
                            <div class="edit-btn-wrp d-flex flex-wrap align-items-center">
                                @if (Laratrust::hasPermission('salesdocument show') ||
                                    ...
                                    @permission('salesdocument show')
                                        <a data-url="{{ route('salesdocument.show', $document->id) }}" data-ajax-popup="true"
                                            data-size="lg" data-bs-whatever="{{ __('Document Details') }}"
        ...
        @permission('salesdocument create')
            <div class="col-xxl-3 col-xl-4 col-sm-6">
                <a href="#" class="btn-addnew-project border-primary p-4" data-ajax-popup="true" data-size="lg"
                    data-title="{{ __('Create Sales Document') }}"
                    data-url="{{ route('salesdocument.create', ['document', 0]) }}">
                    <div class="bg-primary proj-add-icon">
                        <i class="ti ti-plus my-2"></i>
                    </div>
                    <h6 class="my-2 text-center">{{ __('New Document') }}</h6>
                    <p class="text-muted text-center mb-0">{{ __('Click here to add New Document') }}</p>
                </a>
            </div>
        @endpermission
    </div>
    {!! $documents->links('vendor.pagination.global-pagination') !!}
@endsection
