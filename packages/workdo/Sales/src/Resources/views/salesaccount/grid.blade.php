    ...
    <div class="d-flex">
        ...
        @permission('salesaccount import')
            <a href="#" class="btn btn-sm btn-primary me-2" data-ajax-popup="true"
                data-title="{{ __('Sales Account Import') }}" data-url="{{ route('salesaccount.file.import') }}"
                data-toggle="tooltip" title="{{ __('Import') }}"><i class="ti ti-file-import"></i>
            </a>
        @endpermission
        <a href="{{ route('salesaccount.index') }}" class="btn btn-sm btn-primary btn-icon me-2" data-bs-toggle="tooltip"
            title="{{ __('List View') }}">
...
@section('content')
    <div class="row row-gap-2 mb-4">
                    ...
                    <div class="card-header border border-bottom p-3 h-100 d-flex gap-1 justify-content-between">
                        <div class="user-img-wrp d-flex align-items-start w-100">
                            <div class="user-image rounded border-2 border border-primary">
                                <img @if (!empty($account->avatar)) src="{{ !empty($account->avatar) ? asset(Storage::url('upload/profile/' . $account->avatar)) : asset(url('assets/images/user/avatar-2.png')) }}" @else  avatar="{{ $account->name }}" @endif
                                    alt="user-image" class="h-100 w-100">
                            </div>
                            <div class="user-content">
                                @if (\Laratrust::hasPermission('salesdocument edit'))
                                    <h6 class="mb-2"><a class="text-primary"
                                            href="{{ route('salesaccount.edit', $account->id) }}">{{ ucfirst($account->name) }}</a>
                                    </h6>
                                @else
                                    <h6 class="mb-2">{{ ucfirst($account->name) }}</h6>
                                @endif
                                <span class="text-dark text-md">{{ $account->email }}</span>
                            </div>
                                ...
                                @if (Laratrust::hasPermission('salesaccount show') ||
                                    ...
                                    @permission('salesaccount show')
                                        <a href="#" class="btn btn-sm border"
                                            data-url="{{ route('salesaccount.show', $account->id) }}" data-ajax-popup="true"
                                            data-bs-toggle="tooltip" data-bs-original-title="{{ __('View') }}"
                                            data-size="md" data-title="{{ __('Sales Account Details') }}">
                                    ...
                                    @permission('salesaccount edit')
                                        <a href="{{ route('salesaccount.edit', $account->id) }}" data-size="md"
                                            data-title="{{ __('Edit Account') }}" data-bs-original-title="{{ __('Edit') }}"
        ...
        @permission('salesaccount create')
            <div class="col-xxl-3 col-xl-4 col-md-6">
                <a href="#" data-url="{{ route('salesaccount.create', ['account', 0]) }}"
                    class="btn-addnew-project border-primary" data-ajax-popup="true" data-size="lg"
                    data-title="{{ __('Create Sales Account') }}">
                    <div class="bg-primary proj-add-icon">
                        <i class="ti ti-plus my-2"></i>
                    </div>
                    <h6 class="my-2 text-center">{{ __('New Account') }}</h6>
