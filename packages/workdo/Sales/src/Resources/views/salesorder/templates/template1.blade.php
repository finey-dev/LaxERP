...
<head>
    ...
    <title>{{ Workdo\Sales\Entities\SalesOrder::salesorderNumberFormat($salesorder->salesorder_id,$salesorder->created_by,$salesorder->workspace)}} | {{ !empty(company_setting('title_text',$salesorder->created_by,$salesorder->workspace)) ? company_setting('title_text',$salesorder->created_by,$salesorder->workspace) : (!empty(admin_setting('title_text')) ? admin_setting('title_text') :'WorkDo') }}</title>
            ...
            <table>
                <tbody>
                    <tr>
                        ...
                        <td class="text-right">
                            <h3 style="text-transform: uppercase; font-size: 40px; font-weight: bold; ">{{ __('Sales order')}}</h3>
                        </td>
                    </tr>
                </tbody>
            </table>
                        ...
                        <td style="width: 60%;">
                            <table class="no-space">
                                <tbody>
                                    <tr>
                                        <td>{{ __('Number: ')}}</td>
                                        <td class="text-right">{{Workdo\Sales\Entities\SalesOrder::salesorderNumberFormat($salesorder->salesorder_id,$salesorder->created_by,$salesorder->workspace)}}</td>
                                    </tr>
                                    <tr>
                                        <td>{{ __('Issue Date:')}}</td>
                                        <td class="text-right">{{company_date_formate($salesorder->issue_date,$salesorder->created_by,$salesorder->workspace)}}</td>
                                    </tr>
                                    @if(!empty($customFields) && count($salesorder->customField)>0)
                                        @foreach($customFields as $field)
                                            <tr>
                                                <td>{{$field->name}} :</td>
                                                <td class="text-right" style="white-space: normal;"> {{!empty($salesorder->customField[$field->id])?$salesorder->customField[$field->id]:'-'}}</td>
                                            </tr>
                                        @endforeach
                                    @endif
                                    ...
                                    <tr>
                                        <td colspan="2">
                                            <div class="view-qrcode">
                                                <p> {!! DNS2D::getBarcodeHTML(route('pay.salesorder',\Illuminate\Support\Facades\Crypt::encrypt($salesorder->id)), "QRCODE",2,2) !!}
                                            </div>
                                        </td>
                                    </tr>
                                    @endif
            ...
            <table class="add-border invoice-summary" style="margin-top: 30px;">
                ...
                <tbody>
                    ...
                    @foreach($salesorder->items as $key => $item)
                            <tr>
                                <td>{{$item->name}}</td>
                                <td>{{$item->quantity}}</td>
                                <td>{{currency_format_with_sym($item->price,$salesorder->created_by,$salesorder->workspace)}}</td>
                                <td>{{($item->discount!=0)?currency_format_with_sym($item->discount,$salesorder->created_by,$salesorder->workspace):'-'}}</td>
                                <td>
                                ...
                                <td>{{currency_format_with_sym(($item->price * $item->quantity),$salesorder->created_by,$salesorder->workspace)}}</td>
                ...
                <tfoot>
                    <tr>
                        ...
                        <td>{{currency_format_with_sym($salesorder->totalRate,$salesorder->created_by,$salesorder->workspace)}}</td>
                        <td>{{currency_format_with_sym($salesorder->totalDiscount,$salesorder->created_by,$salesorder->workspace)}}</td>
                        <td>{{currency_format_with_sym($salesorder->totalTaxPrice,$salesorder->created_by,$salesorder->workspace) }}</td>
                        <td>{{currency_format_with_sym($salesorder->getSubTotal(),$salesorder->created_by,$salesorder->workspace)}}</td>
                    </tr>
                    <tr>
                        <td colspan="4"></td>
                        <td colspan="2" class="sub-total">
                            <table class="total-table">
                                @if($salesorder->getTotalDiscount())
                                    <tr>
                                        <td>{{__('Discount')}}:</td>
                                        <td>{{currency_format_with_sym($salesorder->getTotalDiscount(),$salesorder->created_by,$salesorder->workspace)}}</td>
                                    </tr>
                                @endif
                                @if(!empty($salesorder->taxesData))
                                    @foreach($salesorder->taxesData as $taxName => $taxPrice)
                                        <tr>
                                            <td>{{$taxName}} :</td>
                                            <td>{{ currency_format_with_sym($taxPrice,$salesorder->created_by,$salesorder->workspace)  }}</td>
                                        </tr>
                                    @endforeach
                                @endif
                                <tr>
                                    <td>{{__('Total')}}:</td>
                                    <td>{{currency_format_with_sym(($salesorder->getSubTotal()-$salesorder->getTotalDiscount()+$salesorder->getTotalTax()),$salesorder->created_by,$salesorder->workspace)}}</td>
                                </tr>
