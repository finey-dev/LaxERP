...
@section('page-action')
    <div class="d-flex">
        ...
        @endpermission
        <a href="{{ route('contact.index') }}" class="btn btn-sm btn-primary btn-icon me-2" data-bs-toggle="tooltip"
            title="{{ __('List View') }}">
...
@section('content')
    <div class="row row-gap-2 mb-4">
                        ...
                        <div class="d-flex align-items-start justify-content-between w-100">
                            <div class="user-img-wrp d-flex align-items-center">
                                <div class="user-image rounded border-2 border border-primary">
                                    <img @if (!empty($contact->avatar)) src="{{ !empty($contact->avatar) ? get_file($contact->avatar) : asset(url('./assets/img/clients/160x160/img-1.png')) }}" @else  avatar="{{ $contact->name }}" @endif
                                        alt="user-image" class="h-100 w-100">
                                </div>
                                <div class="user-content">
                                    @if (\Laratrust::hasPermission('contact edit'))
                                        <h4 class="mb-2 text-break"><a class="text-primary"
                                                href="{{ route('contact.edit', $contact->id) }}">{{ ucfirst($contact->name) }}</a>
                                        </h4>
                                    @else
                                        <h4 class="mb-2">{{ ucfirst($contact->name) }}</h4>
                                    @endif
                                    <span class="text-dark text-md">{{ $contact->email }}</span>
                                </div>
                            </div>
                                ...
                                @if (Laratrust::hasPermission('contact show') ||
                                    ...
                                    @permission('contact show')
                                        <a href="#" class="btn btn-sm border"
                                            data-url="{{ route('contact.show', $contact->id) }}" data-ajax-popup="true"
                                            data-bs-toggle="tooltip" data-bs-original-title="{{ __('View') }}"
                                            data-size="md" data-title="{{ __('Contact Details') }}">
                                    ...
                                    @permission('contact edit')
                                        <a href="{{ route('contact.edit', $contact->id) }}" data-size="md"
                                            class="btn btn-sm border" data-bs-whatever="{{ __('Edit Contact') }}"
        ...
        @permission('contact create')
            <div class="col-xxl-3 col-xl-4 col-md-6">
                <a data-url="{{ route('contact.create', ['contact', 0]) }}" class="btn-addnew-project border-primary"
                    data-ajax-popup="true" data-size="lg" data-title="{{ __('Create Contact') }}">
                    <div class="bg-primary proj-add-icon">
                        <i class="ti ti-plus my-2"></i>
                    </div>
                    <h6 class="my-2 text-center">{{ __('New Contact') }}</h6>
                    <p class="text-muted text-center">{{ __('Click here to add New Contact') }}</p>
                </a>
            </div>
        @endpermission
    </div>
    {!! $contacts->links('vendor.pagination.global-pagination') !!}
@endsection
