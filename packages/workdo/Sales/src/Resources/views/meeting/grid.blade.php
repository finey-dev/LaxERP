...
<div class="d-flex">
    @stack('addButtonHook')
    <a href="{{ route('meeting.index') }}" class="btn btn-sm btn-primary btn-icon me-2" data-bs-toggle="tooltip" title="{{ __('List View') }}">
        <i class="ti ti-list text-white"></i>
    </a>

    @permission('meeting create')
        <a href="#" data-size="lg" data-url="{{ route('meeting.create',['meeting',0]) }}" data-ajax-popup="true" data-bs-toggle="tooltip" data-title="{{__('Create Meeting')}}" title="{{__('Create')}}" class="btn btn-sm btn-primary btn-icon">
...
<div class="row row-gap-2 mb-4">
            ...
            <div class="card user-card">
                    ...
                    <div class="d-flex align-items-center">
                        <div class="user-img-wrp d-flex align-items-center">
                            <div class="user-image rounded border-2 border border-primary">
                                <a>
                                    <img @if(!empty($meeting->avatar)) src="{{(!empty($meeting->avatar))? asset(Storage::url("upload/profile/". $meeting->avatar)): asset(url("./assets/img/clients/160x160/img-1.png"))}}" @else  avatar="{{$meeting->name}}" @endif
                                        alt="user-image" class="h-100 w-100">
                                </a>
                            </div>
                            <div class="user-content">
                                @if (\Laratrust::hasPermission('meeting show'))
                                    <h4 class="mb-2 text-primary text-break"><a class="text-primary" data-url="{{ route('meeting.show', $meeting->id) }}"
                                        href="#" data-ajax-popup="true" data-title="{{ __('Meeting Details') }}">
                                        {{ ucfirst($meeting->name)}}</a>
                                    </h4>
                                @else
                                    <h4 class="mb-2">{{ ucfirst($meeting->name) }}</h4>
                                @endif
                            </div>
                        </div>
                    </div>
                ...
                <div class="card-body text-center p-3">
                    <div class="bottom-icons d-flex flex-wrap align-items-center justify-content-between">
                            ...
                            @if (Laratrust::hasPermission('meeting show') ||
                                ...
                                @permission('meeting edit')
                                    <a href="{{ route('meeting.edit', $meeting->id) }}"
                                        data-bs-whatever="{{ __('Edit Meeting') }}" data-toggle="tooltip"
                                ...
                                @permission('meeting delete')
                                    ...
                                    <a href="#!" class="btn btn-sm border  show_confirm"
                                        ...
                                        data-confirm-yes="delete-form-{{ $meeting['id'] }}">
                                        <svg xmlns="http://www.w3.org/2000/svg" width="16" height="16"
                                            viewBox="0 0 16 16" fill="none">
                        ...
                        @if($meeting->status == 0)
                        <span class="badge bg-success p-2 px-3">{{ __(Workdo\Sales\Entities\Meeting::$status[$meeting->status]) }}</span>
                        @elseif($meeting->status == 1)
                            <span class="badge bg-warning p-2 px-3">{{ __(Workdo\Sales\Entities\Meeting::$status[$meeting->status]) }}</span>
                        @elseif($meeting->status == 2)
                            <span class="badge bg-danger p-2 px-3">{{ __(Workdo\Sales\Entities\Meeting::$status[$meeting->status]) }}</span>
                        @endif
                    </div>
                </div>
            </div>
    ...
    @permission('meeting create')
        <div class="col-xxl-3 col-xl-4 col-sm-6">
            <a data-url="{{ route('meeting.create',['meeting',0]) }}" class="btn-addnew-project border-primary"
                data-ajax-popup="true" data-size="lg" data-title="{{ __('Create Meeting') }}">
                <div class="bg-primary proj-add-icon">
                    <i class="ti ti-plus my-2"></i>
                </div>
                <h6 class="my-2 text-center">{{ __('New Meeting') }}</h6>
                <p class="text-muted text-center">{{ __('Click here to add New Meeting') }}</p>
            </a>
        </div>
    @endpermission
</div>
{!! $meetings->links('vendor.pagination.global-pagination') !!}
@endsection
@push('css')
