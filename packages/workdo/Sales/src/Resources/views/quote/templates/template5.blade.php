...
<head>
    ...
    <title>{{ Workdo\Sales\Entities\Quote::quoteNumberFormat($quote->quote_id, $quote->created_by, $quote->workspace) }}
        |
        {{ !empty(company_setting('title_text', $quote->created_by, $quote->workspace)) ? company_setting('title_text', $quote->created_by, $quote->workspace) : (!empty(admin_setting('title_text')) ? admin_setting('title_text') : 'WorkDo') }}
    </title>
    <link
                    ...
                    <tr>
                        <td style="width: 60%;">
                            <h3
                                ...
                                {{ __('Quote') }}</h3>
                            @if ($settings['quote_qr_display'] == 'on')
                            ...
                            <table class="no-space">
                                <tbody>
                                    <tr>
                                        <td>{{ __('Number: ') }}</td>
                                        <td class="text-right">
                                            {{ Workdo\Sales\Entities\Quote::quoteNumberFormat($quote->quote_id, $quote->created_by, $quote->workspace) }}
                                        </td>
                                    </tr>
                                    <tr>
                                        <td>{{ __('Issue Date:') }}</td>
                                        <td class="text-right">
                                            {{ company_date_formate($quote->issue_date, $quote->created_by, $quote->workspace) }}
                                        </td>
                                    </tr>
                    ...
                    <tr>
                        <td>
                            ...
                            <p>
                                ...
                                {{ !empty($user->bill_state) ? $user->bill_state : '' }},{{ !empty($user->bill_country) ? $user->bill_country : '' }}<br>
            ...
            <table class="add-border invoice-summary" style="margin-top: 30px;">
                    ...
                    @if (isset($quote->items) && count($quote->items) > 0)
                        @foreach ($quote->items as $key => $item)
                            <tr>
                                <td>{{ $item->name }}</td>
                                <td>{{ $item->quantity }}</td>
                                <td>{{ currency_format_with_sym($item->price, $quote->created_by, $quote->workspace) }}
                                </td>
                                <td>{{ $item->discount != 0 ? currency_format_with_sym($item->discount, $quote->created_by, $quote->workspace) : '-' }}
                                </td>
                                <td>
                                ...
                                <td>{{ currency_format_with_sym($item->price * $item->quantity, $quote->created_by, $quote->workspace) }}
                                </td>
                ...
                <tfoot>
                    <tr>
                        ...
                        <td>{{ currency_format_with_sym($quote->totalDiscount, $quote->created_by, $quote->workspace) }}
                        </td>
                        <td>{{ currency_format_with_sym($quote->totalTaxPrice, $quote->created_by, $quote->workspace) }}
                        </td>
                        <td>{{ currency_format_with_sym($quote->getSubTotal(), $quote->created_by, $quote->workspace) }}
                        </td>
                    </tr>
                    <tr>
                        <td colspan="4"></td>
                        <td colspan="2" class="sub-total">
                            <table class="total-table">
                                @if ($quote->getTotalDiscount())
                                    <tr>
                                        <td>{{ __('Discount') }}:</td>
                                        <td>{{ currency_format_with_sym($quote->getTotalDiscount(), $quote->created_by, $quote->workspace) }}
                                        </td>
                                    </tr>
                                @endif
                                @if (!empty($quote->taxesData))
                                    @foreach ($quote->taxesData as $taxName => $taxPrice)
                                        <tr>
                                            <td>{{ $taxName }} :</td>
                                            <td>{{ currency_format_with_sym($taxPrice, $quote->created_by, $quote->workspace) }}
                                            </td>
                                        </tr>
                                    @endforeach
                                @endif
                                <tr>
                                    <td>{{ __('Total') }}:</td>
                                    <td>{{ currency_format_with_sym($quote->getSubTotal() - $quote->getTotalDiscount() + $quote->getTotalTax(), $quote->created_by, $quote->workspace) }}
                                    </td>
                                </tr>
                            </table>
                        </td>
