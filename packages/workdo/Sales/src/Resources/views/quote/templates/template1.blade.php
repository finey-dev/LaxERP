...
<head>
    ...
    <title>{{Workdo\Sales\Entities\Quote::quoteNumberFormat($quote->quote_id,$quote->created_by,$quote->workspace)}} | {{ !empty(company_setting('title_text',$quote->created_by,$quote->workspace)) ? company_setting('title_text',$quote->created_by,$quote->workspace) : (!empty(admin_setting('title_text')) ? admin_setting('title_text') :'WorkDo') }}</title>
                        ...
                        <td style="width: 60%;">
                            <table class="no-space">
                                <tbody>
                                    <tr>
                                        <td>{{ __('Number: ')}}</td>
                                        <td class="text-right">{{Workdo\Sales\Entities\Quote::quoteNumberFormat($quote->quote_id,$quote->created_by,$quote->workspace)}}</td>
                                    </tr>
                                    <tr>
                                        <td>{{ __('Issue Date:')}}</td>
                                    ...
                                    @if(!empty($customFields) && count($quote->customField)>0)
                                        @foreach($customFields as $field)
                                            <tr>
                                                <td>{{$field->name}} :</td>
                                                <td  class="text-right" style="white-space: normal;"> {{!empty($quote->customField[$field->id])?$quote->customField[$field->id]:'-'}}</td>
                                            </tr>
                                        @endforeach
                                    @endif
                                    ...
                                    <tr>
                                        <td colspan="2">
                                            <div class="view-qrcode">
                                                <p>{!! DNS2D::getBarcodeHTML(route('pay.quote',\Illuminate\Support\Facades\Crypt::encrypt($quote->id)), "QRCODE",2,2) !!}
                                            </div>
                                        </td>
                                    </tr>
                                    @endif
                    ...
                    @if(isset($quote->items) && count($quote->items) > 0)
                        @foreach($quote->items as $key => $item)
                            <tr>
                                <td>{{$item->name}}</td>
                                <td>{{$item->quantity}}</td>
                                <td>{{currency_format_with_sym($item->price,$quote->created_by,$quote->workspace)}}</td>
                                <td>{{($item->discount!=0)?currency_format_with_sym($item->discount,$quote->created_by,$quote->workspace):'-'}}</td>
                                <td>
                                ...
                                <td>{{currency_format_with_sym($item->price * $item->quantity,$quote->created_by,$quote->workspace)}}</td>
                        ...
                        <td colspan="2" class="sub-total">
                            <table class="total-table">
                                @if($quote->getTotalDiscount())
                                    <tr>
                                        <td>{{__('Discount')}}:</td>
                                        <td>{{currency_format_with_sym($quote->getTotalDiscount(),$quote->created_by,$quote->workspace)}}</td>
                                    </tr>
                                @endif
                                ...
                                <tr>
                                    <td>{{__('Total')}}:</td>
                                    <td>{{currency_format_with_sym($quote->getSubTotal()-$quote->getTotalDiscount()+$quote->getTotalTax(),$quote->created_by,$quote->workspace)}}</td>
                                </tr>
                            </table>
                        </td>
                    </tr>
