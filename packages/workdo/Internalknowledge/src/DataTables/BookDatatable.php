...
class BookDatatable extends DataTable
{
    ...
    {
        $rowColumn = ['user_id', 'article', 'description'];
        $dataTable =  (new EloquentDataTable($query))
            ->addIndexColumn()
            ->editColumn('user_id', function (Book $book) {
                ...
                foreach ($users as $user) {
                    if (!empty($user->avatar) && file_exists(get_file($user->avatar))) {
                        ...
                        $avatar = asset('packages/workdo/Internalknowledge/src/Resources/assets/image/avatar.png');
                    }
            ...
            ->editColumn('description', function ($book) {
                $url = route('book.description', $book->id);
            ...
            ->addColumn('article', function (Book $book) {
                $articleCount = Article::where('book', $book->id)->count();
                return $articleCount;
            });
        if (\Laratrust::hasPermission('book show') || \Laratrust::hasPermission('book edit') || \Laratrust::hasPermission('book delete')) {
            $dataTable->addColumn('action', function (Book $book) {
                return view('internalknowledge::book.action', compact('book'));
            });
    ...
    public function getColumns(): array
    {
        $column =  [
            ...
            Column::make('user_id')->title(__('Assign User'))->searchable(false)->exportable(false)->printable(false)->orderable(false),
            Column::make('article')->title(__('Article'))->orderable(false)->searchable(false),
