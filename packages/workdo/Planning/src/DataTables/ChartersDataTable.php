...
class ChartersDataTable extends DataTable
{
    /**
    ...
    {
        $rawColumns = ['thumbnail_image','status','stage','challenge','role_id','user_id','rating'];
        $dataTable = (new EloquentDataTable($query))
            ->addIndexColumn()
            ->editColumn('thumbnail_image', function (PlanningCharters $charters) {
                $thumbnail_image = $charters->thumbnail_image ? get_file($charters->thumbnail_image) : asset("packages/workdo/Planning/src/Resources/assets/image/img01.jpg");
            ...
            ->editColumn('status', function (PlanningCharters $charters) {
                return $charters->statuses ? $charters->statuses->name ?? '---' : '---';
            })
            ->editColumn('stage', function (PlanningCharters $charters) {
                return $charters->stages ? $charters->stages->name ?? '---' : '---';
            })
            ->editColumn('challenge', function (PlanningCharters $charters) {
                return $charters->challenges ? $charters->challenges->name ?? '---' : '----';
            })
            ->editColumn('user_id', function (PlanningCharters $charters) {
                ...
                foreach ($users as $user) {
                    $avatar = $user->avatar ? get_file($user->avatar) : get_file('uploads/users-avatar/avatar.png');
        ...
        if ((\Laratrust::hasPermission('charters delete')) || (\Laratrust::hasPermission('charters edit')) || (\Laratrust::hasPermission('charters show'))) {
            $dataTable->addColumn('action', function (PlanningCharters $charters) {
                return view('planning::planningcharter.action', compact('charters'));
            });
    ...
    public function getColumns(): array
    {
        $column = [
            ...
            Column::make('challenge')->title(__('Challenge'))->name('challenges.name'),
