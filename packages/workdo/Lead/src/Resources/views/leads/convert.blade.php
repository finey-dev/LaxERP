    ...
    <div class="modal-body">
        <div class="row">
            ...
            <div class="col-sm-12 col-md-12">
                <div class="d-flex radio-check">
                    <div class="form-check custom-radio custom-control-inline m-1">
                        <input type="radio" name="client_check" value="new" id="new_client" class="form-check-input pointer" @if(empty($exist_client)) checked @endif/>
                        <label class="form-check-label" for="new_client">{{__('New Client')}}</label>
                    </div>
                    <div class="custom-control custom-radio custom-control-inline m-1">
                        <input type="radio" name="client_check" value="exist" id="existing_client" class="form-check-input pointer" @if(!empty($exist_client)) checked @endif/>
                        <label class="form-check-label" for="existing_client">{{__('Existing Client')}}</label>
                    </div>
                </div>
            </div>
            <div class="col-6 exist_client d-none form-group">
                {{ Form::label('clients', __('Client'),['class'=>'col-form-label']) }} <x-required></x-required>
            ...
            <div class="col-6 new_client form-group">
                {{ Form::label('client_name', __('Client Name'),['class'=>'col-form-label']) }} <x-required></x-required>
                {{ Form::text('client_name', $lead->name, array('class' => 'form-control','required'=>'required','placeholder' => __('Enter Client Name'))) }}
            </div>
            <div class="col-6 new_client form-group">
                {{ Form::label('client_email', __('Client Email'),['class'=>'col-form-label']) }} <x-required></x-required>
                {{ Form::text('client_email', $lead->email, array('class' => 'form-control','required'=>'required','placeholder' => __('Enter Client Email'))) }}
            </div>
            <div class="col-6 new_client form-group">
                {{ Form::label('client_password', __('Client Password'),['class'=>'col-form-label']) }} <x-required></x-required>
