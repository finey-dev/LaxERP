...
class RoleDataTable extends DataTable
{
    ...
    public function dataTable(QueryBuilder $query): EloquentDataTable
    {
        return (new EloquentDataTable($query))
            ...
            ->editColumn('permission',function(Role $role){
                ...
                $html = '<div class="fix-show">';
                foreach ($permissions as $permission)
                {
                    if (module_is_active($permission->module) || $permission->module == 'General')
                    {
                        $html .= '<span class="badge p-2 m-1 px-3 bg-primary"> '. $permission->name .'</span>';
                    }
                }
                $html .= '</div><div class="full-show text-primary">(Show More)</div>';
                return $html;
            })
    ...
    {
        return $model->where('created_by', '=', creatorId());
    }
    ...
    public function getColumns(): array
    {
        return [
            ...
            Column::computed('permission')->title(__('Permissions')),
